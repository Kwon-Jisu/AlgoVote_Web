import { Candidate, Pledge, Statement, QnA } from '@/types';

/**
 * 후보자 ID로 후보자 정보 찾기
 * @param id 후보자 ID
 * @returns 후보자 정보 또는 undefined
 */
export const getCandidateById = (id: string): Candidate | undefined => {
  return candidates.find(candidate => candidate.id === id);
};

/**
 * 후보자 ID로 해당 후보의 모든 정보 찾기
 * @param id 후보자 ID
 * @returns 후보자 정보, 공약, 발언, Q&A를 포함한 데이터
 */
export const getCandidateDataById = (id: string) => {
  const candidate = getCandidateById(id);
  const candidatePledges = pledges.filter(pledge => pledge.candidateId === id);
  // const candidateStatements = statements.filter(statement => statement.candidateId === id);
  // const candidateQnAs = qnas.filter(qna => qna.candidateId === id);

  return {
    candidate,
    pledges: candidatePledges,
    // statements: candidateStatements,
    // qnas: candidateQnAs
  };
};

/**
 * 후보자 데이터
 * 모든 페이지에서 재사용 가능한 후보자 정보입니다.
 * 추후 Supabase에서 데이터를 가져오는 것으로 대체될 예정입니다.
 */
export const candidates: Candidate[] = [
  {
    id: 'lee-jaemyung',
    name: '이재명',
    party: '더불어민주당',
    age: 61,
    birthplace: '경상북도 안동군',
    education: ['중앙대학교 법학과 학사', '경원대학교 행정학 석사'],
    career: ['22대 국회의원(인천 계양 을)','6·7대 더불어민주당 대표', '35대 경기도지사', '19·20대 성남시장'],
    slogan: '이제부터 진짜 대한민국',
    profileImage: '/images/candidates/lee-jaemyung.jpg',
    websiteUrl: 'https://theminjoo.kr/main/sub/news/list.php?brd=254',
  },
  {
    id: 'kim-moonsoo',
    name: '김문수',
    party: '국민의힘',
    age: 73,
    birthplace: '경상북도 영천군',
    education: ['서울대학교 경영학 학사'],
    career: ['10대 고용노동부장관', '13대 경제사회노동위워회 위원장', '32·33대 경기도지사'],
    slogan: '새롭게 대한민국, 정정당당 김문수',
    profileImage: '/images/candidates/kim-moonsoo.jpg',
    websiteUrl: 'https://2025victory.kr/sub/?h_page=promise',
  },
  {
    id: 'lee-junseok',
    name: '이준석',
    party: '개혁신당',
    age: 40,
    birthplace: '대구광역시 중구',
    education: ['하버드 대학교 컴퓨터과학·경제학 학사'],
    career: ['22대 국회의원(경기 화성 을)', '초대 개혁신당 대표','초대 국민의힘 대표'],
    slogan: '경제 성장과 사회 안정을 위해',
    profileImage: '/images/candidates/lee-junseok.jpg',
    websiteUrl: 'https://www.reformparty.kr/policy',
  },
];

/**
 * 후보자 공약 데이터
 * 각 후보별 공약 정보를 담고 있습니다.
 */
export const pledges: Pledge[] = [
  // 이재명 후보 공약
  {
    "id": "lee-jaemyung-pledge-1",
    "candidateId": "lee-jaemyung",
    "title": "세계를 선도하는 경제 강국을 만들겠습니다",
    "summary": "",
    "category": "경제·산업",
    "background": [
      "○ AI 등 신산업 집중육성을 통해 새로운 성장기반 구축",
      "○ K-콘텐츠 지원강화로 글로벌 빅5 문화강국 실현"
    ],
    "content": [],
    "implementation": [
      "○ 인공지능 대전환(AX)을 통해 AI 3강으로 도약",
      "- AI 예산 비중 선진국 수준 이상 증액과 민간 투자 100조원 시대 개막",
      "- AI 데이터센터 건설을 통한 ‘AI 고속도로’ 구축 및 국가 혁신거점 육성",
      "- 고성능 GPU 5만개 이상 확보와 국가 AI데이터 집적 클러스터 조성",
      "- ‘모두의 AI’ 프로젝트 추진 및 규제 특례를 통한 AI 융복합 산업 활성화",
      "- AI 시대를 주도할 미래인재 양성 교육 강화",
      "○ 대한민국의 미래성장을 위한 글로벌 소프트파워 Big5 문화강국을 실현",
      "- K컬쳐 글로벌 브랜드화를 통한 K-이니셔티브 실현 및 문화수출 50조원 달성",
      "- K-콘텐츠 창작 전 과정에 대한 국가 지원 강화 및 OTT 등 K-컬처 플랫폼 육성",
      "- 문화예술인의 촘촘한 복지 환경 구축 및 창작권 보장",
      "○ K-방산을 국가대표산업으로 육성",
      "- K방산 수출 증대를 위한 컨트롤타워 신설 및 방위사업청 역량 강화",
      "- 국방 AI 등 R&D 국가 투자 확대 및 방산수출기업 R&D 세제 지원 추진",
      "○ 국가첨단전략산업에 대한 대규모 집중투자방안 마련",
      "- 국민·기업·정부·연기금 등 모든 경제주체들이 참여할 수 있는 국민펀드 조성",
      "- 일반국민·기업의 투자금에 대해 소득세·법인세 감면 등 과감한 세제혜택 부여",
      "- 산업생태계 뒷받침을 위한 기금을 설치하여 맞춤형 자금공급 지원",
      "○ 안정적 R&D 예산 확대 및 국가연구개발 지속성 담보",
      "- 정부 R&D성과가 전체 산업으로 확산되는 혁신성장 체계 구축",
      "- 기초 원천분야 R&D의 안정적 투자",
      "- 혁신성장을 견인할 미래형 창의인재 양성",
      "○ 벤처투자시장 육성으로 글로벌 4대 벤처강국 실현",
      "- 모태펀드 예산 및 벤처·스타트업 R&D 예산 대폭 확대",
      "- M&A 촉진 등을 통한 벤처투자의 회수시장 활성화",
      "- 지역여건을 고려한 스타트업파크 조성, 대학·지식산업센터 등 지역거점으로 육성",
      "○ 스마트 데이터농업 확산, 푸드테크·그린바이오 산업 육성, K-푸드 수출 확대, R&D강화, 농생명용지 조기 개발로 농업을 미래농산업으로 전환·육성"
    ],
    "period": "○ 법률 제·개정 사항은 2025년 6월부터 준비하여 단계적으로 추진\n○ 재정사업은 2025년 추경과 2026년도 예산 수립부터 단계적으로 추진",
    "funding": "○ 정부재정 지출구조 조정분, 2025~2030 연간 총수입증가분(전망) 등으로 충당",
    "order": 1
  },
  {
    "id": "lee-jaemyung-pledge-2",
    "candidateId": "lee-jaemyung",
    "title": "내란극복과 K-민주주의 위상 회복으로 민주주의 강국을 만들겠습니다",
    "summary": "",
    "category": "정치·사법",
    "background": [
      "○ 내란극복",
      "○ 국민통합",
      "○ 민주주의 회복"
    ],
    "content": [],
    "implementation": [
      "○ 대통령 계엄권한에 대한 민주적 통제 강화",
      "- 계엄선포시 국회의 계엄해제권 행사에 대한 제도적 보장 강화",
      "○ 정치보복 관행 근절 등 국민통합 추진",
      "○ 직접민주주의 강화 등을 통한 책임정치 구현",
      "- 국회의원에 대한 국민소환제 도입",
      "○ 국민에 봉사하는 군으로 체질 개선",
      "- 국방문민화 및 군정보기관 개혁",
      "- 3군 참모총장에 대한 인사청문회 도입 및 각 군 이기주의 극복",
      "○ 반인권적으로 운영되고 있는 국가인권위원회의 정상화 추진",
      "- 인권위원장 등 선출시 국민적 후보추천위원회 구성",
      "- 인권위원장과 인권위원의 의무 및 징계규칙 신설",
      "○ 감사원의 정치적 중립성 및 독립성 강화",
      "- 감사개시, 고발여부 결정시 감사위원회 의결 필수화",
      "- 감사원 내부를 감찰하는 감찰관에 감사원 외부인사 임명 의무화",
      "○ 검찰 개혁 완성",
      "- 수사・기소 분리 및 기소권 남용에 대한 사법통제 강화",
      "- 검사 징계 파면 제도 도입",
      "○ 사법 개혁 완수",
      "- 온라인재판 제도 도입, 대법관 정원 확대 등 신속한 재판받을 권리 실질적 보장",
      "- 국민참여재판 확대 등 국민의 사법참여 확대",
      "- 판결문 공개 범위 확대 등 국민의 사법서비스 접근성 제고",
      "○ 반부패 개혁으로 청렴한 공직문화 조성",
      "- 공직자 이해충돌방지 제도 강화",
      "- 고위공직자의 부동산・주식 등 거래내역신고제 도입",
      "○ 주요 공공기관 기관장 등의 임기를 대통령 임기와 일치시켜 공공기관 경영 및 정책 추진의 일관성과 책임성 강화",
      "○ 민생・인권친화적 제도 개선",
      "- 국선변호인 조력 범위의 단계적 확대",
      "- 한국형 디스커버리제도(증거개시제도) 도입, 피해자 진술권 강화 등 사법절차 공정성 확대",
      "○ 변호사의 공공성 강화",
      "- 별도 위원회에 변호사 징계권한 부여 및 변호사 비밀유지권 법제화",
      "○ 방송통신위원회의 정파성 극복을 위한 방송영상미디어 관련 법제 정비",
      "○ 방송의 공공성 회복과 공적책무 이행으로 국민의 방송 실현",
      "- 공영방송의 정치적 독립성 보장을 위한 법제 정비",
      "- 방송의 보도・제작・편성의 자율성 보장",
      "○ 건강하고 신뢰할 수 있는 미디어 이용환경 조성",
      "- 반헌법적・반사회적 콘텐츠에 대한 플랫폼 책임성 강화",
      "- 방송통신심의위원회의 독립성 및 정치적 중립성 강화",
      "○ 제3기 진실화해위원회의 신속 출범",
      "○ 학교 역사교육 강화 및 역사연구기관 운영의 정상화"
    ],
    "period": "○ 법률 제·개정 사항은 2025년 6월부터 준비하여 단계적으로 추진\n○ 재정사업은 2025년 추경과 2026년도 예산 수립부터 단계적으로 추진",
    "funding": "○ 정부재정 지출구조 조정분, 2025~2030 연간 총수입증가분(전망) 등으로 충당",
    "order": 2
  },
  {
    "id": "lee-jaemyung-pledge-3",
    "candidateId": "lee-jaemyung",
    "title": "가계·소상공인의 활력을 증진하고, 공정경제를 실현하겠습니다",
    "summary": "",
    "category": "경제·산업",
    "background": [
      "○ 가계와 소상공인의 활력을 제고",
      "○ 공정한 경제구조 실현"
    ],
    "content": [],
    "implementation": [
      "○ 코로나 정책자금 대출에 대한 채무조정부터 탕감까지 종합방안 마련",
      "○ 12.3 비상계엄으로 인한 피해 소상공인 지원방안 마련",
      "○ 소상공인 금융과 경영부담 완화",
      "- 저금리 대환대출 등 정책자금 확대 및 키오스크 등 각종 수수료 부담 완화",
      "- 건물관리비 내역 공개로 임대료 꼼수 인상 방지",
      "- 디지털 전환 적극 추진 및 글로벌 소상공인 집중 육성",
      "○ 소비촉진으로 소상공인·자영업자 활기 도모",
      "- 지역사랑상품권 및 온누리상품권 발행 규모 확대",
      "- 지역별 대표상권 및 소규모 골목상권 육성을 통한 상권르네상스 2.0 추진",
      "○ 공정하고 지속가능하며 실패해도 재기할 수 있는 소상공인 경제 구축",
      "- ‘소상공인 내일채움공제’ 도입으로 목돈 마련 기회 제공",
      "- 폐업지원금 현실화 및 폐업시 대출금 일시상환 유예 요건 완화",
      "○ 소상공인·자영업자 사회안전망 확대",
      "- 경찰청 연계 안심콜 의무화로 여성 소상공인 안전 강화",
      "- 소상공인·자영업자 육아휴직수당 확대",
      "- 자영업자의 ‘아프면 쉴 권리’를 위한 상병수당 확대",
      "○ 가맹점주·대리점주·수탁사업자·온라인플랫폼 입점사업자 등 협상력 강화",
      "○ 플랫폼 중개수수료율 차별금지 및 수수료 상한제 도입으로 공정한 배달문화 구축",
      "○ 대환대출 활성화 및 중도상환수수료 단계적 감면 등 대출상환 부담 완화",
      "○ 취약계층에 대한 중금리대출 전문 인터넷은행 추진",
      "○ 가산금리 산정 시 법적비용의 금융소비자 부당전가 방지로 원리금상환부담 경감",
      "○ 채무자 중심의 보호체계 구축 및 사각지대 해소",
      "- 장기소액연체채권 소각 등을 위한 배드뱅크 설치",
      "- 특별감면제·상환유예제 등 청산형 채무조정 적용 확대",
      "○ 고품질 공공임대주택 및 공공임대 비율 단계적 확대",
      "○ 전세사기 걱정 없는 사회, 부담없는 전월세로 서민의 주거사다리 복원",
      "- 전세사기 걱정 없고 임차인에게 책임이 전가되지 않는 보증제도 개선",
      "- 월세 세액공제 대상자 및 대상주택 범위 확대 등 월세 부담 완화",
      "○ 주식시장 수급여건 개선 및 유동성 확충",
      "- 상장기업 특성에 따른 주식시장 재편 및 주주환원 강화",
      "- 외국인 투자자 유입 확대를 위한 제도 정비 및 MSCI 선진국지수 편입 적극 추진",
      "○ 디지털자산 생태계 정비를 통한 산업육성기반 마련",
      "○ 중소기업협동조합 등 단체협상권 부여로 제값받는 공정한 경제 창출",
      "○ 중소기업 복지플랫폼 예산 확대 및 중소기업 상생금융지수 도입 추진",
      "○ 한국형 디스커버리제도 도입 등 기술탈취 행위 강력 근절",
      "○ 상법상 주주충실 의무 도입 등 기업지배구조 개선 통한 일반주주의 권익 보호",
      "○ 자본·손익거래 등을 악용한 지배주주의 사익편취 행위 근절",
      "○ 먹튀·시세조종 근절로 공정한 시장질서 창출"
    ],
    "period": "○ 법률 제·개정 사항은 2025년 6월부터 준비하여 단계적으로 추진\n○ 재정사업은 2025년 추경과 2026년도 예산 수립부터 단계적으로 추진",
    "funding": "○ 정부재정 지출구조 조정분, 2025~2030 연간 총수입증가분(전망) 등으로 충당",
    "order": 3
  },
  {
    "id": "lee-jaemyung-pledge-4",
    "candidateId": "lee-jaemyung",
    "title": "세계질서 변화에 실용적으로 대처하는 외교안보 강국을 만들겠습니다",
    "summary": "",
    "category": "외교·통상",
    "background": [
      "○ 튼튼한 경제안보 구축",
      "○ 지속가능한 한반도 평화 실현"
    ],
    "content": [],
    "implementation": [
      "○ 국제적 통상환경 변화에 적극 대응하는 경제외교 추진",
      "- G20, G7 등을 통한 글로벌 현안 적극 참여",
      "- 성공적인 2025 경주 APEC 개최를 통한 외교역량 강화 및 국제위상 제고",
      "- 경제안보 증진을 위한 주요국과의 연대 강화",
      "○ 우리의 외교영역을 확대하고 다변화",
      "- 신아시아 전략 및 글로벌사우스 협력 추진",
      "- 통상·공급망·방산·인프라 등 분야에 있어 EU 및 유럽과의 실질협력 강화",
      "○ 통상환경 변화 대응을 위한 무역구조 혁신",
      "- 국익을 최우선할 수 있도록 산업경쟁력 제고 및 전략적 통상정책 추진",
      "- 수출시장·품목 다변화 추진 및 기후위기발 글로벌 환경무역 대응역량 강화",
      "- 핵심소재·연료광물의 공급망 안정화를 위한 통상협력 강화",
      "○ 국익과 실용의 기반 하에 주변 4국과의 외교관계 발전",
      "○ 북한 핵 위협의 단계적 감축 및 비핵·평화체제를 향한 실질적 진전 달성",
      "○ 한반도 군사적 긴장 완화, 평화 분위기 조성",
      "- 한반도 비핵화 목표 아래 남북관계 복원 및 화해·협력으로의 전환 추진",
      "- 우발적 충돌방지 및 군사적 긴장완화, 신뢰구축 조치 추진",
      "○ 굳건한 한미동맹에 기반한 전방위적 억제능력 확보",
      "- 한국형 탄도미사일 성능 고도화 및 한국형 미사일방어체계 고도화",
      "- 한미동맹 기반 하 전시작전권 환수 추진",
      "○ 국제사회에서의 공헌과 국격에 걸맞은 외교 추진",
      "- UN 등 국제기구에서의 적극적 역할 확대, 글로벌 기후위기 대응외교 강화",
      "- 국제개발협력 및 적극적 공공외교 추진",
      "○ 이산가족 상봉 등 남북 인도주의협력, 교류협력 모색·추진",
      "○ 재외국민에 대한 적극적인 지원을 통한 권익과 안전 보호",
      "○ 주력 제조업 경쟁력 강화를 위한 ‘전략산업 국내생산 촉진세제’ 도입",
      "○ 수출산업 보호를 위한 무역안보 단속체계 확립",
      "- 국산 둔갑 우회수출, 국가 핵심기술 유출 단속 강화",
      "- 공급망 위기를 선제적으로 포착할 수 있는 공급망 조기경보시스템 고도화",
      "○ 전략물자 국적선박 확보를 통한 물류 안보 실현",
      "○ 식량안보 차원에서 쌀 등 주요농산물의 안정적 공급기반 구축",
      "○ 어업협정 이행 강화 및 ‘불법 중국어선 강력 대응’을 통한 해양 주권 수호"
    ],
    "period": "○ 법률 제·개정 사항은 2025년 6월부터 준비하여 단계적으로 추진\n○ 재정사업은 2025년 추경과 2026년도 예산 수립부터 단계적으로 추진",
    "funding": "○ 정부재정 지출구조 조정분, 2025~2030 연간 총수입증가분(전망) 등으로 충당",
    "order": 4
  },
  {
    "id": "lee-jaemyung-pledge-5",
    "candidateId": "lee-jaemyung",
    "title": "국민의 생명과 안전을 지키는 나라를 만들겠습니다",
    "summary": "",
    "category": "사법·행정·보건의료",
    "background": [
      "○ 국민 생활안전 및 재난 대응 강화",
      "○ 의료 대란 해결 및 의료 개혁 추진"
    ],
    "content": [],
    "implementation": [
      "○ 범죄로부터 안전한 사회 구축",
      "- 흉악범죄·묻지마범죄(이상동기범죄) 예방을 위해 범죄경력자 관리 감독 강화",
      "- 교제폭력 범죄 처벌 강화 및 피해자 보호명령제도 도입",
      "- 청소년 범죄 대응체계 강화",
      "○ 민생침해 금융범죄 처벌 및 금융소비자 보호를 위한 제도 개선 추진",
      "- 민생파괴 금융범죄에 대한 처벌 대폭 강화",
      "- 보이스피싱·다중사기범죄 등 서민 다중피해범죄에 대한 범죄이익 몰수",
      "- 금융사고 책임자 엄정처벌 및 금융보안 의무위반 징벌적 과징금 부과",
      "○ 사회적 재난에 대한 신속하고 효율적인 대응체계 구축",
      "- 재난현장지휘권 강화로 대규모 재난 신속대응 및 부처별 협업체계 구축",
      "- 사회재난 발생 시 사고조사위원회 즉시 설치",
      "- 재난안전 산업 육성 및 소방·화재 안전 장비 확충",
      "- 산불‧호우·땅꺼짐 사고를 포함한 통합 기후재난 예측·감시시스템 도입",
      "- 생명안전기본법 제정 추진",
      "○ 교통사고 예방과 건설안전 환경조성으로 생활안전 체계 강화",
      "- 보행자 안전 우선의 교통체계 구축으로 교통취약계층 우선보호",
      "- 오토바이 전후방 번호판제 도입 및 고령운전자 운전 안전 대책 마련",
      "- 전기차 배터리 인증제 활성화 및 전기차 화재예방·진압장비 도입 확대",
      "- 공유형 전동킥보드 안전관리 강화 및 효율적 이용을 위한 관련법 제정",
      "- 항공사고 예방을 위한 항공사·공항시설 안전관련 투자·정비 점검 강화",
      "- 건설공사 발주ㆍ설계ㆍ시공ㆍ감리 등 전 과정에 대한 안전대책 강화",
      "○ 지역·필수·공공의료 강화로 제대로 치료받을 권리 확보",
      "- 필수의료에 대한 충분한 보상체계 확립 및 의료사고에 대한 국가책임 강화",
      "- 지역의사·지역의대·공공의료사관학교 신설로 지역·필수·공공의료 인력 확보",
      "- 진료권 중심 공공의료 인프라 확충 및 국립대병원 거점병원 역할 강화",
      "- 응급환자 신속 이송-수용-전원체계로 응급실 뺑뺑이 문제 해결",
      "- 진료권 중심 응급의료체계와 중증-응급 24시간 전문의 대응체계 확립",
      "- 주치의 중심 맞춤형 일차 의료체계 구축 및 방문·재택 진료 확대",
      "- 의료의 질과 안전성을 고려한 비대면 진료 제도화",
      "- 필수의약품 수급불안 해소와 감염병 위기 대응 인프라 구축",
      "○ 국민참여 의료개혁으로 의료대란 해결, 건강보험의 지속가능성 확보",
      "- ‘국민참여형 의료개혁 공론화위원회’로 국민이 원하는 진짜 의료개혁 추진",
      "- 보건의료 전문직역들의 상호협력체계 강화 및 적정인력 확보",
      "- 건강보험에 대한 안정적인 국고지원 및 수가보상체계의 합리적 개편",
      "- 대상별·질환별 특성 고려한 보장성 확대로 의료비 절감과 질병 예방",
      "- 희귀·난치 질환 부담 완화와 소아비만·소아당뇨에 대한 국가책임 강화"
    ],
    "period": "○ 법률 제·개정 사항은 2025년 6월부터 준비하여 단계적으로 추진\n○ 재정사업은 2025년 추경과 2026년도 예산 수립부터 단계적으로 추진\n○ 건강보험 관련 사항은 사회적 논의 통해 건보재정 계획 마련",
    "funding": "○ 정부재정 지출구조 조정분, 2025~2030 연간 총수입증가분(전망) 등으로 충당. 건강보험 재정 관련 사항은 별도 논의 필요",
    "order": 5
  },
  {
    "id": "lee-jaemyung-pledge-6",
    "candidateId": "lee-jaemyung",
    "title": "세종 행정수도와 ‘5극 3특’ 추진으로 국토균형발전을 이루겠습니다",
    "summary": "",
    "category": "행정·경제·산업",
    "background": [
      "○ 세종 행정수도 완성",
      "○ 5극(5대 초광역권: 수도권, 동남권, 대경권, 중부권, 호남권), 3특(3대 특별자치도(제주, 강원, 전북)) 추진"
    ],
    "content": [],
    "implementation": [
      "○ 세종 행정수도 완성 추진",
      "- 국회 세종의사당과 대통령 세종 집무실 임기 내 건립",
      "○ 이전 공공기관 정주여건 개선 및 제2차 공공기관 지방이전 추진",
      "○ 5극, 3특 중심 균형발전 기반 마련",
      "- 5대 초광역권별 특별지방자치단체 구성 및 권역별 광역급행철도 건설",
      "- 3대 특별자치도의 자치권한 및 경쟁력 강화 위한 특별법 개정",
      "○ 자치분권 강화와 지방재정 확충",
      "- 국가자치분권회의 신설 추진",
      "- 지방교부세 확대, 자체세원 발굴 등으로 지방재정 확충",
      "○ 지역소멸을 방지하기 위한 지역 주도 행정체계 개편 추진",
      "- 범부처 통합 TF구성 및 로드맵 마련",
      "- 주민의사를 반영한 지자체 통합방안 마련",
      "○ 지역대표 전략산업 육성과 지역투자 촉진",
      "- 혁신도시·경제자유구역·국가/지방산단 연계한 전략산업 육성",
      "- 위기산업 구조개혁 및 지역산업 생태계 안정 도모",
      "○ 수도권 중심의 대학 서열화 완화를 위한 지역 거점국립대 전략투자",
      "- ‘서울대 10개 만들기’",
      "- 국립대-사립대 동반성장 위한 RISE 체계 구축",
      "○ 지역사랑상품권 발행 의무화로 지역경제 및 균형발전 실현",
      "○ 관광산업 활성화",
      "- 국민휴가 지원 3종 세트 확대",
      "- 지자체·기업 워케이션 및 지역특화 관광자원 개발",
      "○ 지속가능한 농산어촌 조성",
      "- 주거여건 개선, 빈집 정비, 귀농·귀촌 지원 강화",
      "- 청년 농업·어업·임업 인재 육성",
      "○ 철도지하화 대상 구간 추진 위한 종합계획 수립",
      "○ 지역·중소방송사 콘텐츠 제작 지원 확대",
      "○ 이·통장 특별활동비 신설"
    ],
    "period": "○ 법률 제·개정 사항은 2025년 6월부터 준비하여 단계적으로 추진\n○ 재정사업은 2025년 추경과 2026년도 예산 수립부터 단계적으로 추진",
    "funding": "○ 정부재정 지출구조 조정분, 2025~2030 연간 총수입증가분(전망) 등으로 충당",
    "order": 6
  },
  {
    "id": "lee-jaemyung-pledge-7",
    "candidateId": "lee-jaemyung",
    "title": "노동이 존중받고 모든 사람의 권리가 보장되는 사회를 만들겠습니다",
    "summary": "",
    "category": "교육·경제·복지",
    "background": [
      "○ 노동 존중, 일하는 사람들의 권리 존중 사회 실현"
    ],
    "content": [],
    "implementation": [
      "○ 자영업자, 특수고용 및 플랫폼 노동자 등 일하는 모든 사람들의 일터 권리 보장",
      "- 일한 만큼 보상받는 공정한 노동환경 조성",
      "- 법제도 개선 및 미조직 취약계층 권리 대변 강화",
      "- 「노동조합법」 2·3조 개정으로 하청노동자 등의 교섭권 보장",
      "○ 포괄임금제 금지 명문화",
      "○ 동일노동 동일임금 기준지표 마련 위한 임금분포제 도입",
      "○ 산업·업종·지역단위 단체교섭협약 활성화",
      "- 저임금노동자 노동조건 보장",
      "- 공공부문이 모범 사용자로 법 준수 및 협약모델 구축",
      "○ 직장 내 민주주의, 노사자율 강화",
      "- 근로자 대표 선출·임기·역할·법적 보호 제도화",
      "○ 고위험 자영업자 산재보험 제도 도입",
      "○ 노동안전보건체계 강화",
      "- 원·하청 통합 안전보건관리체계 구축",
      "○ 일하는 여성을 위한 사회 조성",
      "- 고용평등 임금공시제 도입",
      "- 공공기관 성별 평등지표 반영",
      "○ 주4.5일제 도입 및 노동시간 단축 로드맵 수립",
      "- 2030년까지 OECD 평균 이하 목표",
      "○ 공무원 처우 및 공직문화 개선",
      "- 저연차 공무원 보수 인상, 위험근무수당 인상",
      "- 불합리한 공직 관행 개선",
      "○ 문화예술인 창작권 보장",
      "- 정부의 침해 금지",
      "○ 장애인 권리보장 및 사회참여 확대",
      "- ‘장애인권리보장법’ 제정",
      "- 교통약자 교통수단 확대 및 발전계획 마련"
    ],
    "period": "○ 법률 등 제·개정 사항은 2025년 6월부터 준비하여 단계적 추진\n○ 재정사업은 2025년 제2회추경안, 2026년도 예산안 편성부터 단계적 추진",
    "funding": "○ 정부재정 지출구조 조정분, 2025~2030 연간 총수입증가분(전망) 등으로 충당",
    "order": 7
  },
  {
    "id": "lee-jaemyung-pledge-8",
    "candidateId": "lee-jaemyung",
    "title": "생활안정으로 아동·청년·어르신 등 모두가 잘사는 나라를 만들겠습니다",
    "summary": "",
    "category": "경제·복지",
    "background": [
      "○ 생활안정과 생활비절감 추진",
      "○ 빈틈없이 기본이 보장되는 사회 추진"
    ],
    "content": [],
    "implementation": [
      "○ 생애주기별 소득보장체계 구축",
      "- 아동수당 지급 대상을 18세까지 점진적 상향",
      "- 일하는 모든 취업자로 육아휴직 단계적 확대",
      "- 국민연금 사각지대 해소 및 연금개혁 지속 추진",
      "○ 온 사회가 다 같이 돌보는 돌봄기본사회 추진",
      "- 영유아 교육·보육비 지원 확대 및 온동네 초등돌봄체계 구축",
      "- 간병비 부담완화와 간호·간병 통합병동 확대 추진",
      "- 지속 가능한 지역사회 통합 돌봄체계 구축",
      "○ 근로장려금(EITC)과 자녀장려금(CTC)의 대상 및 지급액 확대",
      "○ 주거·통신 등 필수적인 생활비 부담 절감",
      "- 월세 세액공제 대상자·대상주택 확대 및 통신비 세액공제 신설",
      "○ 청년·국민·어르신 패스 3종 도입으로 국민 교통비 절감",
      "○ 국가책임 공교육으로 사교육비 부담 경감",
      "- 기초학력 학습안전망 지원 확대 및 자기주도학습센터 운영",
      "○ 대학생 등록금 부담 완화 및 청년주거 환경 개선",
      "○ 생애주기 문화패스 신설·확대 등 국민 문화향유권 확대",
      "○ 선진국형 농가소득 및 농업재해안전망 도입",
      "- 농산물 가격 안정적 관리 및 농어업재해 국가책임 강화",
      "- 양곡관리법 개정 등을 통한 쌀값 정상화 및 공익직불금 확대",
      "- 농업인 퇴직연금제 도입 및 농지이양 은퇴직불금·공공비축농지 확대",
      "○ 어민 소득증대 통한 어촌소멸 대응",
      "- 탄소중립 활동 참여 어촌마을 안정적인 소득기반 마련",
      "- 수산식품기업바우처 및 수산선도조직 육성사업 지원 확대",
      "○ 국가유공자 예우 강화 및 보훈문화 확산",
      "- 저소득 보훈대상자에 대한 지원체계 강화 및 사각지대 없는 보훈의료체계 구축",
      "○ 문화예술인 사회보험보장 확대 및 복합지원공간 확충",
      "○ 청년의 기회와 복지 확대",
      "- ‘청년미래적금’ 도입 등 청년자산형성 지원",
      "- 학자금 대출 소득요건 완화 및 이자면제 확대",
      "- 군복무 경력 호봉 반영, 구직활동지원금 확대 등 청년 일할 권리 강화",
      "- 청년 맞춤형 공공분양 및 월세지원 확대",
      "- 국민연금 군복무 크레딧 확대 등 청년생활안전망 구축",
      "- 글로벌 기업 연계 직업교육 프로그램 확산·지원",
      "○ 1인가구·청년 주거정책 확대",
      "- 주거복합플랫폼주택 조성 및 맞춤형 주거설계 지원",
      "○ 한부모가족 복지급여 확대 등 안정적 생활환경 조성",
      "○ 교통물류 환경 개선",
      "- 근로여건 개선 및 국민편익 향상",
      "○ 청년·근로자 천원의 아침밥 및 농식품바우처 확대",
      "○ 사람과 동물이 더불어 행복한 사회 조성",
      "- 반려동물 의료비 지원 및 학대방지"
    ],
    "period": "○ 법률 제·개정 사항은 2025년 6월부터 준비하여 단계적으로 추진\n○ 재정사업은 2025년 추경과 2026년도 예산 수립부터 단계적으로 추진",
    "funding": "○ 정부재정 지출구조 조정분, 2025~2030 연간 총수입증가분(전망) 등으로 충당",
    "order": 8
  },
  {
    "id": "lee-jaemyung-pledge-9",
    "candidateId": "lee-jaemyung",
    "title": "저출생·고령화 위기를 극복하고 아이부터 어르신까지 함께 돌보는 국가를 만들겠습니다",
    "summary": "",
    "category": "교육·복지",
    "background": [
      "○ 저출생·고령화 해소 및 돌봄체계 구축"
    ],
    "content": [],
    "implementation": [
      "○ 저출생 대책 혁신 및 자녀양육 지원 확대",
      "- 자녀 수에 비례한 신용카드 소득공제율‧공제 한도 상향 추진",
      "- 초등학생 예체능학원‧체육시설 이용료를 교육비 세액공제 대상에 추가",
      "- ‘우리아이자립펀드’ 단계적 도입 및 신혼부부 결혼출산지원 확대",
      "- 신혼부부 공공임대주택 공급 확대",
      "- 난임부부 치료지원 강화",
      "○ 아이 키우기 좋은 나라를 위한 돌봄‧교육, 일·가정 양립 지원 강화",
      "- 공공 아이돌봄 서비스 지원 강화",
      "- 지자체 협력형 초등돌봄 추진",
      "- 초등학교 방과후학교 수업료 지원 확대",
      "- 교육·보육의 질을 높이는 정부 책임형 유보 통합 추진",
      "○ 발달장애인 24시간 돌봄 등 장애인 맞춤형 지역돌봄체계 구축",
      "○ 생애주기별 외로움(고독) 대응 정책 개발‧추진",
      "○ 고령사회 대응을 위한 통합적 지원체계 마련",
      "- 치매‧장애 등으로 재산 관리가 어려운 노인을 위한 공공신탁제도 도입",
      "- 어르신 주거 문제 해결을 위한 고령자 친화 주택·은퇴자 도시 조성",
      "- 간호·간병 통합서비스 확대 및 요양병원 간병비 건강보험 적용",
      "- 노인 등이 집에서 의료‧돌봄서비스를 받는 지역사회 통합 돌봄체계 구축",
      "○ 지속 가능한 노후 소득 보장 체계 구축",
      "- 국민연금 수급 연령에 맞춘 정년 연장, 사회적 합의를 통해 단계적 추진",
      "- 주택연금 제도개선 등을 통해 노후 소득 보장을 위한 지원강화"
    ],
    "period": "○ 법률 제·개정 사항은 2025년 6월부터 준비하여 단계적으로 추진\n○ 재정사업은 2025년 추경과 2026년도 예산 수립부터 단계적으로 추진",
    "funding": "○ 정부재정 지출구조 조정분, 2025~2030 연간 총수입증가분(전망) 등으로 충당",
    "order": 9
  },
  {
    "id": "lee-jaemyung-pledge-10",
    "candidateId": "lee-jaemyung",
    "title": "미래세대를 위해 기후위기에 적극 대응하겠습니다",
    "summary": "",
    "category": "환경·산업",
    "background": [
      "○ 기후위기 대응 및 산업구조의 탈탄소 전환"
    ],
    "content": [],
    "implementation": [
      "○ 선진국으로서의 책임에 걸맞는 온실가스 감축목표 수립",
      "- 2030년 온실가스 감축 목표 달성 추진과 과학적 근거에 따른 2035년 이후 감축 로드맵 수립",
      "- 탄소중립기본법 개정",
      "- 2028년 제33차 기후변화협약 당사국총회(COP33) 유치",
      "○ 재생에너지 중심의 에너지전환 가속화",
      "- 2040년까지 석탄화력발전 폐쇄",
      "- 햇빛·바람 연금 확대, 농가태양광 설치로 에너지 자립 실현",
      "- 태양광 이격거리 규제 개선 및 PPA 제도 개선",
      "○ 에너지고속도로 구축",
      "- 서해안 및 한반도 에너지 인프라 구축",
      "- 지능형 전력망 시스템 마련",
      "○ 탄소중립 산업전환 및 신산업 육성",
      "- 태양광·풍력·전기차·배터리·수전해·히트펌프 등 국산화 및 수출경쟁력 확보",
      "- RE100 산업단지 조성",
      "- 탄소다배출 업종 저탄소 공정 혁신",
      "- 기후테크 R&D 및 신산업 발굴 강화",
      "○ 건축·수송·농축산업 부문 탈탄소화",
      "- 노후건물 그린리모델링, 전기차 보급 확대, 노후경유차 대·폐차 지원",
      "- 영농형태양광 보급, 유기농업 및 지속가능 축산 확대",
      "○ 자원순환 및 생물다양성 복원",
      "- 탈플라스틱 로드맵 및 바이오플라스틱 육성",
      "- 생물다양성보호구역 확대 및 4대강 재자연화",
      "○ 국민참여 기후대응 강화",
      "- 탄소포인트제 인센티브 확대",
      "○ 정의로운 전환 기반 마련",
      "- 배출권 유상할당 비중 확대",
      "- 정의로운 전환 특구 및 고용전환 지원",
      "○ 2028년 제4차 UN해양총회 유치"
    ],
    "period": "○ 법률 제·개정 사항은 2025년 6월부터 준비하여 단계적으로 추진\n○ 재정사업은 2025년 추경과 2026년도 예산 수립부터 단계적으로 추진",
    "funding": "○ 정부재정 지출구조 조정분, 2025~2030 연간 총수입증가분(전망) 등으로 충당",
    "order": 10
  },
  
  // 김문수 후보 공약
  {
    "id": "kim-moonsoo-pledge-1",
    "candidateId": "kim-moonsoo",
    "title": "자유 주도 성장, 기업하기 좋은 나라",
    "summary": "자본·기술·노동 혁신, 규제완화와 세제정비로 기업투자 활성화",
    "category": "경제·산업",
    "background": [
      "○ 자본, 기술, 노동 3대 혁신으로 경제를 대전환하여 함께 잘사는 새로운 성장과 번영의 시대 실현",
      "○ 민간 및 기업의 자율성과 창의성을 극대화하여 ‘자유 주도 성장’ 달성",
      "○ 규제 완화, 세제 정비, 투자 활성화를 통해 기업하기 좋은 나라, 일자리 창출",
      "○ 글로벌 경쟁력 강화, 신산업·신기술 성장 추진"
    ],
    "content": [],
    "implementation": [
      "○ 세계 최고 수준의 투자 환경 조성",
      "- 투자 저해 규제는 글로벌 스탠다드 수준으로 혁신",
      "- 경쟁국보다 앞선 기업환경 조성으로 기업투자 활성화",
      "- 「자유경제혁신 기본법」 제정으로 신기술‧신산업 분야 규제 철폐, 미래산업 육성",
      "- 기업투자 종합지원센터 설립으로 원스톱 행정서비스 제공",
      "- 인프라, 세제, 보조금 등 기업투자 인센티브 확대",
      "○ 기업하기 좋은 나라, 일하기 좋은 나라",
      "- 취임 즉시 한미정상회담 개최 및 관세 패키지 협상 추진",
      "- 대통령 주재 수출진흥회의 정례화, 해외 투자자 대상 기업설명회(IR) 개최",
      "- 노사합의 기반 주52시간제 근로시간 개선",
      "- 원자력 발전 확대를 통한 산업용 전기료 인하 추진",
      "- 법인세 및 상속세 최고세율 인하 등 세제 정비",
      "- 일자리 창출 우수기업에 세금 및 부담금 감면 우대"
    ],
    "period": "○ 취임 즉시 추진",
    "funding": "○ 기존 재원 활용으로 추가 재정 소요 없음",
    "order": 1
  },
  {
    "id": "kim-moonsoo-pledge-2",
    "candidateId": "kim-moonsoo",
    "title": "AI·에너지 3대 강국 도약",
    "summary": "AI 전 주기 투자, 원자력 중심 에너지 확대, 과학기술 초격차 실현",
    "category": "과학기술·정보통신",
    "background": [
      "○ AI 전 주기에 걸친 집중 투자와 생태계 조성으로 3대 강국 도약",
      "○ 세계 1위 원자력 기술로 에너지 강국 실현",
      "○ 과학기술인이 존중받는 나라로 초격차 기술 혁신"
    ],
    "content": [],
    "implementation": [
      "○ AI 청년 인재 20만명 양성",
      "- AI 대학원, SW 중심대학 정원 확대, 글로벌 대학 협력",
      "- 해외 우수인력 유치 지원(정부-민간 매칭)",
      "- 전 국민 대상 디지털 리터러시 확산",
      "○ AI 유니콘 기업 육성",
      "- 민관합동 100조 펀드 조성, 핵심기술 인프라 확보",
      "○ AI 융합센터 및 생태계 강화",
      "- 공공데이터 개방, 차세대 AI 기술개발(NPU, GPU, HBM)",
      "- AI 스타트업·벤처 성장펀드 조성, 민간 발굴·육성",
      "○ AI 규제 글로벌 스탠다드화",
      "- 국가AI위원회 기능 강화, AI 정책보좌관 신설, 민간전문가 위촉",
      "- 데이터 규제 혁신, 기준국가제 적용",
      "○ AI 산업용 전력 확보",
      "- 대형 원전 6기 차질 없이 추진, 소형원전(SMR) 상용화",
      "- 원전 비중 확대, 산업용 전기료 인하",
      "- 에너지 고속도로 연계 및 신기술 개발",
      "○ 과학기술기본법 개정, 부총리·특임대사 신설",
      "○ 과학기술인 처우 개선",
      "- 연구자 정년 65세 회복, 연봉 표준 상향, 공제회 지원 강화",
      "- 학생연구자 처우 개선",
      "○ 연구 중심 허브 및 몰입 환경 조성",
      "- 5대 첨단기술 국제공동연구센터, 성실한 실패 인증제",
      "- 성과 통합 관리 시스템 구축"
    ],
    "period": "○ 2025년부터 추진",
    "funding": "○ 국비, 민간 투자, 글로벌 기업 유치",
    "order": 2
  },
  {
    "id": "kim-moonsoo-pledge-3",
    "candidateId": "kim-moonsoo",
    "title": "청년이 크는 나라, 미래가 열리는 대한민국",
    "summary": "청년의 취업, 주거, 결혼, 육아 부담 완화 및 기회 창출",
    "category": "청년·복지",
    "background": [
      "○ 청년의 미래를 키우는 나라",
      "○ 청년 취업, 주거, 교육, 결혼, 양육 부담 완화로 청년을 위한 기회 창출"
    ],
    "content": [],
    "implementation": [
      "○ 청년안심 국민연금 2차 개혁",
      "- 연금위원회 청년세대 참여 보장, 자동조정장치 도입 등",
      "○ 청년 일자리 창출",
      "- 대기업 신입 공채 장려, 창업지원, 스타트업 빌리지 조성",
      "- 청년인재 및 무역전문인력 양성",
      "○ 주거 지원",
      "- 3·3·3 청년주택: 결혼·출산 시 9년간 지원 주택 연 10만호 공급",
      "- 공공기숙사, 반값월세존 확대 등",
      "○ 결혼과 육아 부담 완화",
      "- 결혼서비스업 표준계약서, 공공예식장 인프라",
      "- 소득세 감세, 보육수당 비과세 확대, ‘첫걸음 계좌’ 도입",
      "- 24시간·긴급돌봄시설 확대, 0세아 1:1 보육",
      "○ 청년 도약 지원",
      "- 생활비 대출 확대, 도약장려금·계좌·저축공제 확대",
      "○ 군가산점제 및 여성희망복무제 도입",
      "- 군 복무 경력의 민간 활용 확대, 자격연계 및 학점은행제 운영"
    ],
    "period": "○ 2025년부터 추진",
    "funding": "○ 국비 및 지방비, 기존 예산 재조정, 주택도시기금 등 공공기금 활용",
    "order": 3
  },
  {
    "id": "kim-moonsoo-pledge-4",
    "candidateId": "kim-moonsoo",
    "title": "GTX로 연결되는 나라, 함께 크는 대한민국",
    "summary": "GTX 전국 확대, 초광역권 메가시티 추진, 교통혁신과 균형발전",
    "category": "주거·교통",
    "background": [
      "○ GTX를 수도권에서 전국 5대 광역권으로 확장하여 국토 균형발전 촉진",
      "○ 지역균형발전과 미래 전략산업 활성화를 위한 초광역권 메가시티 추진",
      "○ 수도권 교통 체증 해소 및 통근 시간 단축으로 국민 삶의 질 개선"
    ],
    "content": [],
    "implementation": [
      "○ GTX 전국 5대 광역권 확대",
      "- 수도권 GTX 노선 A~F 적시 개통, 울산~목포까지 전국 급행철도망 구축",
      "○ 광역철도, 도시철도 확충",
      "- 30분 출퇴근 혁명, 정주환경 개선 위한 교육·의료·문화 인프라 확충",
      "○ 초광역권 메가시티 조성",
      "- 수도권, 충청권, 호남권, 대경권, 동남권에 성장기반 조성",
      "○ 메가프리존 도입",
      "- 기존 특구제도 통합, 지자체 기획 기반 규제 완화 및 성장 엔진 설치",
      "○ 중앙정부 권한 지방 이양 확대",
      "- 도시계획, 산업, 교육, 세제, 입법 등 중앙 권한의 지자체 이양 확대",
      "- 2차 공공기관 이전, 국회 완전 이전, 대통령 제2 집무실 이전"
    ],
    "period": "○ 취임 즉시 추진",
    "funding": "○ 민자 유치, 국비·지방비, SOC 예산 재조정",
    "order": 4
  },
  {
    "id": "kim-moonsoo-pledge-5",
    "candidateId": "kim-moonsoo",
    "title": "중산층 자산증식, 기회의 나라",
    "summary": "중산층 감세, 자산형성 지원, 상속세제 개편, 민생비용 완화",
    "category": "경제·산업",
    "background": [
      "○ 중산층 비율을 늘리고, 소득과 자산을 증가시켜 생활 수준 향상"
    ],
    "content": [],
    "implementation": [
      "○ 중산층 감세 추진",
      "- 종합소득세 물가연동제, 기본공제·경로우대·장애인 공제 상향",
      "○ 자산형성 지원",
      "- 장기주식보유 세제 혜택, 가상자산 ETF 허용, ISA 확대",
      "○ 상속세제 개편",
      "- 부부간 상속세 폐지, 유산취득세 방식으로 전환, 최고세율 30% 인하",
      "○ 생활물가 부담 완화",
      "- K-원패스, 영유아용품 면세, 농산물 직거래 확대",
      "○ 부동산 제도 개편",
      "- 화이트존 도입, 종부세·양도세 개편, 재개발·재건축 권한 이양",
      "- 재건축 초과이익환수제 폐지"
    ],
    "period": "○ 취임 즉시 실시, 국회 통과 즉시 시행",
    "funding": "○ 재정 소요 없음. 세제 개편 및 규제 완화로 경제성장과 세수 확대",
    "order": 5
  },
  {
    "id": "kim-moonsoo-pledge-6",
    "candidateId": "kim-moonsoo",
    "title": "아이 낳고 기르기 좋은 나라, 안심되는 평생복지",
    "summary": "출산·육아·교육·돌봄 확대 및 의료안전망 강화",
    "category": "복지·보건",
    "background": [
      "○ 인구구조 변화에 맞는 결혼·임신·출산·육아·교육 및 어르신 돌봄 강화",
      "○ 가족 친화 생애 맞춤형 복지로 안심되는 평생 복지 구현",
      "○ 안심하고 누리는 더 좋은 보건의료"
    ],
    "content": [],
    "implementation": [
      "○ 청년·신혼부부 주거지원, 내 집 마련 지원",
      "- 9년간 주거비 지원, 신생아 특례 대출 및 조건 완화",
      "○ 임신부터 육아까지 전방위 지원",
      "- 난임 시술 건강보험 포함, 분만비·산후조리 지원, 보건지소 확대",
      "○ 아동 자산 형성",
      "- 0~17세 계좌 신설, 디딤씨앗통장 확대",
      "○ 디딤돌 소득 및 복지사각지대 해소",
      "○ 예방접종 확대 및 방문접종 지원",
      "- 고령자·청소년 독감, 폐렴구균, HPV 등 확대 지원",
      "○ 어르신 돌봄 및 건강 지원",
      "- 일자리 확대, 요양병원 간병비 보험적용, 치매 돌봄 강화",
      "○ 장애인 생애주기별 지원",
      "- 돌봄 확대, 건강권 보장, 원스톱 지원센터 설치",
      "○ 의료안전망 복구 및 의료개혁 재검토",
      "- 6개월 내 의료시스템 재건, 미래의료위원회 설치"
    ],
    "period": "○ 2025년부터",
    "funding": "○ 예산 우선순위 조정, 기금 및 국비·지방비 활용",
    "order": 6
  },
  {
    "id": "kim-moonsoo-pledge-7",
    "candidateId": "kim-moonsoo",
    "title": "소상공인, 민생이 살아나는 서민경제",
    "summary": "소상공인 응급지원, 금융 확대, 부담 경감, 플랫폼 경쟁력 강화",
    "category": "경제·산업",
    "background": [
      "○ 지속가능한 소상공인 경제 생태계 실현",
      "○ 서민경제 회복 및 안전망 강화",
      "○ 생활필수품 가격안정화 등 민생경제 활성화"
    ],
    "content": [],
    "implementation": [
      "○ 대통령 직속 ‘소상공인 자영업자 지원단’ 설치",
      "○ 소상공인 응급지원 3대 패키지",
      "- 생계방패 특별융자, 경영안정자금 확대, 새출발 희망프로젝트 강화",
      "○ 금융 지원 확대",
      "- 소상공인 전문은행 신설, 금융 플랫폼 통합, 맞춤형 상품 확대",
      "○ 공과금·전기료 등 부담 경감",
      "- 바우처 지원, 보험 지원, 신용카드 발급 프로그램 제공",
      "○ 상가임대차 보호 및 관리비 투명화 시스템 구축",
      "○ 플랫폼 경쟁력 강화 및 디지털 마케팅 지원",
      "○ 신용카드 캐시백, 소비 촉진 캠페인, 소비 포인트 시스템",
      "○ 전통시장 활성화",
      "- 온누리상품권 증액, 주차 및 교통 편의 시설 확충, 세액공제 확대"
    ],
    "period": "○ 2025년부터",
    "funding": "○ 재정 재조정, 국비·지방비 활용",
    "order": 7
  },
  {
    "id": "kim-moonsoo-pledge-8",
    "candidateId": "kim-moonsoo",
    "title": "재난에 강한 나라, 국민을 지키는 대한민국",
    "summary": "재난 사전대비 중심 전환, 기후재난 대응, 재난정보 통합",
    "category": "행정·정치개혁",
    "background": [
      "○ 복합·대형화되는 재난에 대응력을 강화하고 사전대비 중심으로 전환",
      "○ 예측 → 대응 → 복구까지 전 주기적 재난관리 체계 구축"
    ],
    "content": [],
    "implementation": [
      "○ 사후 수습에서 사전 대비로 전환",
      "- 재난 통합시스템, 통합정보공유 플랫폼, AI 기반 위험지도 작성",
      "○ 재난전문조직 현장 대응역량 강화",
      "- 드론·로봇 감시, 재난 장비 확충, 전문 조직 활성화",
      "○ 환경부를 기후환경부로 개편",
      "- 부처 통합조정권 강화, 기후예측 시스템 고도화",
      "○ 재난정보 통합 플랫폼 구축 및 민간 협력 강화",
      "○ 항공 안전체계 개편",
      "- 운영·건설 주체 분리, 지방항공 통합안전기준 정비",
      "○ 싱크홀 대응체계 구축 및 R&D 확대",
      "○ 하천지류 정비사업 추진 및 노후 제방 안전 강화"
    ],
    "period": "○ 2025년부터",
    "funding": "○ 재난 관련 기금 확충, 첨단기술 R&D 투자, 예산 우선순위 재조정",
    "order": 8
  },
  {
    "id": "kim-moonsoo-pledge-9",
    "candidateId": "kim-moonsoo",
    "title": "특권을 끊는 정부, 신뢰를 세우는 나라",
    "summary": "정치·사법 개혁 및 감사제도 정비, 공수처 폐지, 대공수사 강화",
    "category": "행정·정치개혁",
    "background": [
      "○ 정치·사법·선거관리 제도의 개혁을 통해 국민 신뢰 회복",
      "○ 독립적 감사제도를 확립하여 공직사회 부정부패 일소"
    ],
    "content": [],
    "implementation": [
      "○ 감사원 소속 감사관 파견·임명",
      "- 전 부처, 지자체, 공공기관 감사 투명성 확보",
      "○ 국회의원 불체포 특권 폐지",
      "○ 선관위 외부 감사 허용을 위한 감사원법 개정",
      "○ 고위공직자범죄수사처(공수처) 폐지",
      "○ 사법 방해죄 신설",
      "- 수사·재판 방해행위 처벌 신설",
      "○ 대공수사권 국정원 환원",
      "- 사이버 방첩, 정보보호 시스템 구축",
      "○ 간첩죄 관련 형법 개정",
      "- ‘적국’ 대신 ‘외국 또는 외국인 단체’를 포함, 정보범위 확장"
    ],
    "period": "○ 2025년부터",
    "funding": "○ 재원 소요 없음",
    "order": 9
  },
  {
    "id": "kim-moonsoo-pledge-10",
    "candidateId": "kim-moonsoo",
    "title": "북핵을 이기는 힘, 튼튼한 국가안보",
    "summary": "핵억제력, 국방력, 글로벌 방산 및 병영환경 개선",
    "category": "외교·안보",
    "background": [
      "○ 북핵 억제력 강화로 국민이 안심하는 국방 구현",
      "○ 미국과의 신뢰 관계를 바탕으로 ‘강한 대한민국’, ‘국민이 안심하는 대한민국’, ‘국제사회가 신뢰하는 대한민국’ 구현",
      "○ 북핵 위협에 흔들리지 않는 대한민국 구현"
    ],
    "content": [],
    "implementation": [
      "○ 한미동맹 기반 핵 확장억제 실행력 강화",
      "- 전략자산 상시 주둔 수준, 핵 재래식 통합훈련 내실화, 방위조약 개정",
      "○ 한국형 3축 체계 강화",
      "- 킬체인 보완, 스카이돔 구축, 보복 수단 확보",
      "○ 핵 잠재력 확보 및 전술핵 재배치 협의",
      "- 핵무기 설계기술, 우라늄·플루토늄 기술, NATO식 핵공유",
      "○ 핵추진 잠수함 개발",
      "○ 사이버전자전사령부 창설",
      "○ 글로벌 K-방산 육성",
      "- 제도·금융지원, 10대 국방첨단기술 집중 육성",
      "○ 병영환경 개선 및 장병 처우 향상",
      "- 수당 현실화, 급식 품질 향상, 초급간부 처우 개선",
      "○ 복무 중 사상자 보상 및 보훈 확대"
    ],
    "period": "○ 2025년부터",
    "funding": "○ 국방 예산 혁신 및 재조정, 장병 복지 투자 우선순위 반영",
    "order": 10
  },
  
  // 이준석 후보 공약
  {
    "id": "lee-junseok-pledge-1",
    "candidateId": "lee-junseok",
    "title": "대통령 힘빼고 일 잘하는 정부 만든다",
    "summary": "작은 정부, 부처 간 통합, 실무 중심 조직 개편",
    "category": "행정",
    "background": [
      "부처 간 소관 분야 중복과 행정의 칸막이 문제를 최소화하여 효율적이고 전문화된 정부 운영",
      "실무 중심의 작은 정부 기조 확립 및 부처 이름이 아닌 실제 업무 성과와 전문성을 중시하는 문화 정착"
    ],
    "content": [],
    "implementation": [
      "① 부처 개편 및 축소 (19부처 → 13부처)",
      " - 유사·중복 업무 부처 통폐합",
      "   · 교육부와 과학기술정보통신부 통합 (교육과학부)",
      "   · 통일부 폐지 및 외교부로 업무 통합 (외교통일부)",
      "   · 여성가족부 폐지 및 관련 업무 복지부와 내무부(행정안전부)로 이관",
      "   · 보건부를 별도 분리 신설하여 보건의료 분야 전문성 강화",
      "   · 국가보훈부를 복지부로 개편하여 복지정책과 통합 운영",
      "   · 산업자원통상부와 중소기업벤처부 통합하여 산업에너지부로 일원화",
      "   · 국토교통부, 환경부, 해양수산부의 해양 업무를 건설교통부로 통합하고, 수산 분야는 일차산업부로 통합",
      "   · 문화체육관광부, 기획재정부, 행정안전부를 각각 문화부, 재정경제부, 내무부로 명칭 변경",
      "② 3부총리제 도입",
      " - 안보부총리, 전략부총리, 사회부총리를 각각 임명하여 책임운영체제 구축",
      " - 대통령 산하 국가안보실 폐지, 안보부총리가 해당 기능 수행",
      "③ 예산 편성 기능 이관",
      " - 기획재정부에서 예산기획 기능을 분리하여 국무총리실 산하 '예산기획실' 신설",
      " - 각 부처 예산 총액 배정 국무회의에서 의결하여 특정 부처의 예산 독점 방지",
      "④ 정부기구 효율화",
      " - 고위공직자범죄수사처(공수처) 폐지",
      " - 국가인권위원회와 국민권익위원회 통합하여 기능적 효율성 극대화"
    ],
    "period": "◦ 부처 개편 계획 수립 및 관련 법령 개정안 마련: 취임 후 6개월 내\n◦ 국회 법령 개정 및 예산 심의 통과: 취임 후 1년 이내\n◦ 통합·신설 부처 운영 및 정착: 취임 후 2년 이내 완료",
    "funding": "◦ 부처 및 위원회 통합을 통한 중복 예산 절감\n◦ 행정 운영 효율화를 통한 예산 절약 및 부처 간 예산 배정 투명성 강화로 추가 재정 투입 최소화\n◦ 축소된 조직 운영을 통해 절감된 인건비와 운영비를 개편 비용으로 활용, 추가 예산 편성 최소화",
    "order": 1
  },
  {
    "id": "lee-junseok-pledge-2",
    "candidateId": "lee-junseok",
    "title": "중국·베트남 공장을 다시 대한민국으로",
    "summary": "리쇼어링 유도, 외국인 노동자 제도 개선, 산업단지 활성화",
    "category": "산업자원",
    "background": [
      "해외로 이전한 국내 기업의 국내 주요 산업단지 리쇼어링 촉진을 통해 지역 경제 활성화 및 러스트벨트 해소",
      "해외 현지 노동조건을 일정 기간 유지할 수 있도록 외국인 노동자 임금 차등적용 및 규제 완화를 통해 기업의 국내 복귀 유인 극대화"
    ],
    "content": [],
    "implementation": [
      "① 리쇼어링 기업 지원을 위한 특별 규제완화",
      " - 울산미포, 여수, 반월-시화, 온산, 창원, 구미 등 주요 국가산단 복귀 기업 대상",
      " - 외국인 노동자 최저임금 차등 적용 기간: 최대 10년",
      "② 외국인 노동자 전용 특수 비자 신설",
      " - 산단 특수 비자(E-9-11) 신설, 고용 절차 간소화",
      " - 쿼터 운영으로 신속 인력 충원",
      "③ 외국인 노동자 정착 지원 강화",
      " - 언어 및 문화적응 교육 지원"
    ],
    "period": "◦ 세부 시행방안 및 법령 개정안 준비: 취임 후 3개월 내\n◦ 관련 법 개정 및 제도 정비 완료: 6개월 이내\n◦ 제도 본격 운영: 1년 이내",
    "funding": "◦ 장기적으로 세수 확대 기대\n◦ 단기적으로 기존 산업단지 지원 및 교육 예산 재편성으로 충당",
    "order": 2
  },
  {
    "id": "lee-junseok-pledge-3",
    "candidateId": "lee-junseok",
    "title": "지자체, 법인세 자치권 부여로 지방 경쟁력 강화!",
    "summary": "법인세 일부 지방세 전환, 지자체 세율 자율화, 지방 경쟁력 제고",
    "category": "조세 지방자치",
    "background": [
      "지방자치단체의 법인세 자치권을 확대하여 지역 경제 활성화 및 지자체 간 경쟁을 촉진",
      "지역 간 재정 자립도 제고 및 실질적인 지방 분권 강화"
    ],
    "content": [],
    "implementation": [
      "① 법인세 국세분의 일부를 지방세로 전환",
      " - 법인세 국세 30% 감면 후, 전액 지방세 전환",
      " - 지자체가 탄력세율 (최대 50%) 설정 가능",
      "② 지자체별 법인세 경쟁 체계 구축",
      " - 세율 자율 결정 및 시행예고제 도입",
      "③ 지방소비세 이전 효과 극대화",
      " - 법인 본사 지방 이전 시 소비세 재정 효과 증가",
      " - 필요시 소비세 비율 상향"
    ],
    "period": "◦ 법 개정 준비: 취임 후 6개월 내\n◦ 국회 통과: 1년 이내\n◦ 제도 전면 시행: 2년 이내",
    "funding": "◦ 지방교부금 조정으로 별도 예산 투입 없이 운영 가능",
    "order": 3
  },
  {
    "id": "lee-junseok-pledge-4",
    "candidateId": "lee-junseok",
    "title": "최저임금 최종 결정 권한 지자체에 위임",
    "summary": "지자체 최저임금 결정권 도입으로 지역경제 자율성 확대",
    "category": "경제/산업",
    "background": [
      "지역 간 생활비 및 인건비 격차를 고려한 최저임금제도 개편으로 지방자치단체의 자율성을 강화하고 지역경제 활성화 촉진",
      "실질적인 지방자치와 지역 상황에 맞는 노동정책 구현"
    ],
    "content": [],
    "implementation": [
      "① 최저임금 결정 권한의 지방 이양",
      " - 중앙정부 최저임금 기준을 바탕으로, 지자체가 ±30% 내 가감 결정 가능",
      "② 지역 실정 반영한 제도 운영",
      " - 광역지방의회가 주거비, 생활비, 기업 인건비 등을 반영해 결정",
      "③ 적용 기준 명확화",
      " - 근로자의 실질 근무지를 기준으로 적용",
      " - 모호한 경우는 중앙정부 기준 적용"
    ],
    "period": "◦ 법령 개정안 마련: 취임 후 6개월 이내\n◦ 제도 정비 및 시행 준비: 1년 이내\n◦ 본격 시행: 2년 이내",
    "funding": "◦ 기존 중앙·지방 예산 내 가능\n◦ 지역경제 활성화에 따른 세수 증가로 재정 강화 기대",
    "order": 4
  },
  {
    "id": "lee-junseok-pledge-5",
    "candidateId": "lee-junseok",
    "title": "국민연금, 신-구 연금 분리가 유일한 해결책",
    "summary": "신·구 연금 체계 분리 및 자동조정장치 도입으로 연금 지속 가능성 확보",
    "category": "복지/보건",
    "background": [
      "국민연금 구조개혁을 통한 지속 가능한 연금체계 구축 및 세대 간 형평성 확보",
      "‘낸 만큼은 반드시 받는’ 공정하고 안정적인 연금제도 확립"
    ],
    "content": [],
    "implementation": [
      "① 신-구 연금 재정 분리",
      " - 개혁 시점 이후 보험료는 신연금 계정으로 별도 관리",
      "② 구연금 미적립부채 관리",
      " - 자동조정장치 조기 시행 및 국고 조기 투입",
      " - 연기제도, 반납·추납 등 과지급 제도 개선",
      "③ 확정기여형 구조의 신연금 도입",
      " - 납입 보험료에 따라 급여 결정",
      " - 기대 수익비를 1로 조정"
    ],
    "period": "◦ 구조개혁 법안 준비: 취임 후 6개월 내\n◦ 제도 구축 완료: 1년 이내\n◦ 완전 시행 및 정착: 2년 이내",
    "funding": "◦ 자동조정장치와 국고 조기 투입으로 국고 부담 최소화\n◦ 신연금은 추가 재정 투입 없이 운영 가능",
    "order": 5
  },
  {
    "id": "lee-junseok-pledge-6",
    "candidateId": "lee-junseok",
    "title": "교권 보호를 위한 교사 소송 국가책임제 및 학습지도실 제도 도입",
    "summary": "교사 소송 국가책임제, 허위신고 처벌, 학습지도실 설치",
    "category": "교육",
    "background": [
      "교사에 대한 무분별한 소송과 허위신고로부터 교권과 학습권을 보호",
      "교실 내 수업 방해와 문제행동에 즉각 대응할 수 있는 제도 마련으로 수업권과 학습권 보장"
    ],
    "content": [],
    "implementation": [
      "① 교사 소송 국가책임제 및 전담 변호사 배치",
      " - 직무 관련 민형사 소송에 대해 교육청이 직접 대리",
      " - 시도교육청에 전담 변호사 배치",
      "② 허위신고 무고 처벌 강화",
      " - 허위 아동학대 신고자에 민사상 손해배상 및 형사처벌 부과",
      "③ 학습지도실 및 학생생활지원관 제도 도입",
      " - 학교당 학습지도실 1곳 이상 설치, 지원관 배치",
      "④ 단계적 분리조치 및 적응 프로그램",
      " - 수업방해 발생 시 분리조치, 재적응 프로그램 제공"
    ],
    "period": "◦ 법령 및 시행령 개정안 마련: 취임 후 3개월 내\n◦ 인력 배치 계획 수립: 6개월 이내\n◦ 제도 시행 및 전면 적용: 1년 이내",
    "funding": "◦ 교육청 예산 내 인건비 항목 재편성\n◦ 학습지도실 설치는 교육부 예산 내 우선 배정\n◦ 허위신고 감소로 행정·수업 손실 비용 절감",
    "order": 6
  },
  {
    "id": "lee-junseok-pledge-7",
    "candidateId": "lee-junseok",
    "title": "5천만원 한도 든든출발자금으로 청년의 도전 응원!",
    "summary": "청년에게 용도 제한 없는 저금리 대출 제공, 사회진입 지원",
    "category": "경제/산업",
    "background": [
      "자산 형성과 사회 진출의 시기에 있는 청년에게 초기 자금 지원을 통해 실질적 기회의 평등 실현",
      "보호종료아동, 고졸청년, 대학중퇴자 등 제도 사각지대 청년에게 균등한 출발 기반 제공"
    ],
    "content": [],
    "implementation": [
      "① 정책금융상품 ‘든든출발자금’ 출시",
      " - 만 19~34세 대상, 용도 제한 없음",
      " - 최대 5천만원, 연 1.7% 고정금리 대출",
      "② 상환 방식의 유연성 보장",
      " - 일반상환: 5년 거치, 10년 분할 / 취업후상환 가능",
      "③ 학자금대출과의 선택 및 전환 허용",
      " - 상호 전환 가능, 추가 대출 가능",
      "④ 용도 제한 및 사후 관리 체계",
      " - 불법 용도 제한, 지출 증빙 요구"
    ],
    "period": "◦ 정책 설계 및 시스템 구축: 취임 후 6개월 이내\n◦ 한국장학재단 등과 협업 출시: 1년 이내",
    "funding": "◦ 기존 청년 정책 및 학자금대출 예산 재편성\n◦ 실효성 낮은 사업 정비로 재정 효율성 확보\n◦ 상환 전제 구조로 국가재정 순손실 최소화",
    "order": 7
  },
  {
    "id": "lee-junseok-pledge-8",
    "candidateId": "lee-junseok",
    "title": "현역대상자 가운데 장교 선발한다",
    "summary": "병사 중심 장교·부사관 선발, 복무 연계 등록금 지원",
    "category": "외교/안보/통일",
    "background": [
      "병사 중심의 장교·부사관 선발 구조를 통해 인력 기반 확대 및 복무 유인 강화",
      "군 복무와 학업 연계를 보장하여 효율적인 군 인력 운영과 청년 지원 동시 달성"
    ],
    "content": [],
    "implementation": [
      "① 병사·부사관·장교 통합 기초군사훈련",
      " - 전원 4주 훈련, 우수자 선발",
      "② 단기복무 간부 선발제도 도입",
      " - 장교(상위 10%), 부사관(10~25%) 선발",
      " - 각 4개월 교육, 총 2년 복무, 필요시 6개월 연장 가능",
      "③ 복무 연계 등록금 지원",
      " - 복무기간에 비례한 대학/대학원 등록금 전액 지원(국공립 기준)",
      " - 중도 포기 시 지원금 회수",
      "④ 복무 유예제 도입",
      " - 기초군사훈련 수료 후 최대 3년 복무 유예 가능"
    ],
    "period": "◦ 군 인사법 정비: 6개월 이내\n◦ 시범사업 및 인프라 구축: 1년 이내\n◦ 전면 시행: 2년 이내",
    "funding": "◦ 기존 병력운영 예산 재배치\n◦ 등록금 상한 설정으로 안정적 운용\n◦ ROTC 미달 등으로 인한 간접비용 감소 기대",
    "order": 8
  },
  {
    "id": "lee-junseok-pledge-9",
    "candidateId": "lee-junseok",
    "title": "압도적 규제 혁파 위한 ‘규제기준국가제’ 실시",
    "summary": "글로벌 규제 수준 벤치마킹, 규제심판원 신설",
    "category": "경제/산업",
    "background": [
      "글로벌 선도국가의 규제 수준을 벤치마킹하여 국내 규제를 획기적으로 혁신",
      "창업과 신산업의 개척을 가로막는 낡은 규제를 정비하고 예측 가능하고 일관된 기업환경 조성"
    ],
    "content": [],
    "implementation": [
      "① 규제기준국가제 도입",
      " - 기준국가 규제 사례 제시 시 특별 허가 부여",
      "② 규제심판원 신설",
      " - 국무총리 산하 설치, 원스톱 처리",
      "③ 특례 기간 확대 및 안전장치 마련",
      " - 최대 10년, 인명피해 시 중단 가능",
      "④ 단체 중심 집단 신청 허용",
      " - 협회/단체 신청 가능, 단체 가입 기업은 특례 적용",
      "⑤ 분야별 기준국가 설정 및 개혁 로드맵 마련",
      " - 클라우드, 핀테크, 자율주행 등"
    ],
    "period": "◦ 법제화 및 기관 설립: 6개월 이내\n◦ 기준국가 선정 및 시범사업: 1년 이내\n◦ 5개년 로드맵 추진: 2년 이내",
    "funding": "◦ 기존 규제샌드박스 및 부처 규제개선 예산 활용\n◦ 민간투자 활성화 및 행정절차 축소로 자생적 재정효과 기대",
    "order": 9
  },
  {
    "id": "lee-junseok-pledge-10",
    "candidateId": "lee-junseok",
    "title": "‘과학기술 성과연금’ 및 ‘과학자 패스트트랙’ 등 국가과학영웅 우대제도 도입",
    "summary": "성과기반 연구자 연금 및 출입국 패스트트랙 제공",
    "category": "교육/과학기술",
    "background": [
      "우수 과학기술인을 국가 차원에서 예우함으로써 연구자 자부심 회복 및 인재 유출 방지",
      "연구성과에 기반한 명예 보상과 편의 지원을 통해 연구 몰입 환경 조성"
    ],
    "content": [],
    "implementation": [
      "① 과학기술 연구자 연금 도입",
      " - 노벨상·필즈상·훈장 수상자 및 고성과 논문 게재자 대상",
      " - 성과 등급별 월 최대 500만원 연금",
      " - 기준심의위원회에서 분야별 심사",
      "② 과학기술 패스트트랙 제도 도입",
      " - SCI Q1 논문, 국제학회 발표 등 기준 충족 시",
      " - 외교관 수준의 출입국 우대"
    ],
    "period": "◦ 연금 기준 및 평가체계 설계: 6개월 내\n◦ 법령 정비 및 협의 완료: 1년 내\n◦ 제도 시행 및 수급자 선정: 2년 내",
    "funding": "◦ 연구포상·관리비 등 기존 예산 통합\n◦ 일정 성과만 대상이므로 예산 절감\n◦ 패스트트랙은 행정 지원으로 예산 부담 최소",
    "order": 10
  },
];

/**
 * 정책 카테고리 설명
 * 각 정책 카테고리에 대한 설명을 제공합니다.
 */
export const categoryDescriptions = {
  "경제·산업": "기업 환경, 규제 개혁, 산업 성장 전략, 지역 산업 활성화, 공정경제 및 경제 안보 등 경제 전반을 다루는 정책",
  "교육·과학기술": "전략기술 투자, 연구 인재 육성, 교권 보호 및 교육 혁신 등 교육 및 과학기술 분야에 대한 정책",
  "사회통합": "청년, 노동권, 군복무 등 사회적 통합과 권리 보장을 다루는 정책",
  "주거·교통": "광역 및 생활교통망 확장, 교통 혼잡 해소, 주거 안정 관련 정책",
  "복지·보건": "출산·돌봄, 보건의료 개혁, 연금 및 기초생활 보장 등 사회 복지 및 보건 시스템 강화 정책",
  "행정·정치개혁": "재난안전 체계, 정치·사법 제도 개혁, 지방분권 및 행정 효율화 등 행정 및 정치 제도 개선 정책",
  "외교·안보": "국방 및 안보 강화, 외교 및 평화 전략 등 국가 외교 및 안보 정책"
};

/**
 * 정책 서브 카테고리 정의
 * 정책 카테고리에 대한 세부 카테고리 목록
 */
export const subCategories = {
  "경제·산업": [
    "기업환경 및 규제개혁",
    "산업·성장 전략",
    "지역 불균형 해소",
    "중산층·가계 지원",
    "소상공인·서민경제 지원",
    "콘텐츠·문화산업 육성",
    "공정경제·경제안보",
    "리쇼어링·지역 산업 활성화"
  ],
  "교육·과학기술": [
    "전략기술 투자",
    "연구·인재 육성",
    "교권·교육"
  ],
  "사회통합": [
    "청년 지원 및 기회 보장",
    "노동권 및 권리 존중",
    "군복무 제도 및 병역 연계 개선"
  ],
  "주거·교통": [
    "광역교통망 확장",
    "생활교통·혼잡 해소"
  ],
  "복지·보건": [
    "출산·돌봄 복지",
    "보건의료 개혁",
    "연금·기초보장"
  ],
  "행정·정치개혁": [
    "재난·생활안전 체계 정비",
    "정치·사법제도 개혁 및 신뢰 회복",
    "지방분권 및 지역 균형 강화",
    "정부조직 개편 및 행정 효율화"
  ],
  "외교·안보": [
    "국방·안보 강화",
    "외교·평화 전략"
  ]
};


// * 후보자별 서브 카테고리 정책 데이터
//  * 각 후보의 서브 카테고리별 정책 내용을 담고 있습니다.
//  */
export const subCategoryPolicies = {
  "kim-moonsoo": {
    "경제·산업": {
      "기업환경 및 규제개혁": {
        "title": "규제 완화, 세제 정비, 투자 활성화를 통해 기업하기 좋은 나라, 일자리 창출",
        "content": "조성, 투자 중심의 정책 추진",
        "details": [
          "세계 최고 수준의 투자 환경 조성"
        ]
      },
      "산업·성장 전략": {
        "title": "민간 및 기업의 자율성과 창의성을 극대화하여 ‘자유 주도 성장’ 달성",
        "content": "조성, 투자 중심의 정책 추진",
        "details": [
          "기업하기 좋은 나라, 일하기 좋은 나라",
          "세계 최고 수준의 투자 환경 조성"
        ]
      },
      "지역 불균형 해소": {
        "title": "지역균형발전과 미래 전략산업 활성화를 위한 초광역권 메가시티 추진",
        "content": "조성, 도입, 확대 중심의 정책 추진",
        "details": [
          "초광역권 메가시티 조성",
          "미래첨단산업기반 마련을 위한 메가프리존 도입",
          "중앙정부의 권한과 자원 지방 이양 확대"
        ]
      },
      "중산층·가계 지원": {
        "title": "중산층 비율을 늘리고, 소득과 자산을 증가시켜 생활 수준 향상",
        "content": "지원 중심의 정책 추진",
        "details": [
          "중산층 감세 추진",
          "중산층 자산형성 지원",
          "시대 변화에 맞게 상속세제 개편",
          "생활물가 부담 완화",
          "부동산 제도 개편"
        ]
      },
      "소상공인·서민경제 지원": {
        "title": "지속가능한 소상공인 경제 생태계 실현",
        "content": "강화, 지원, 확대 중심의 정책 추진",
        "details": [
          "대통령 직속으로 가칭 “소상공인 자영업자 지원단” 설치",
          "소상공인 응급 지원 3대 패키지 시행",
          "서민·소상공인 금융 지원 확대",
          "소상공인 전기료 등 부담 경감, 맞춤형 지원 강화",
          "상가임대차보호대상 확대 및 관리비 투명화 시스템 구축으로 분쟁 예방",
          "소상공인 플랫폼 시장 경쟁력 강화, 디지털 마케팅 지원",
          "소상공인 매장 신용카드 캐시백 제공 등 내수 활성화",
          "지역소비 촉진과 전통시장 활성화 확대"
        ]
      }
    },
    "교육·과학기술": {
      "전략기술 투자": {
        "title": "AI 전 주기에 걸친 집중 투자와 생태계 조성으로 3대 강국 도약",
        "content": "강화, 지원 중심의 정책 추진",
        "details": [
          "AI 청년 인재 20만명 양성",
          "AI 유니콘 기업 지원",
          "글로벌 최첨단 AI 융합센터 구축으로 AI 생태계 강화",
          "AI 관련 규제 글로벌 스탠더드 수준으로 혁신",
          "AI 산업 필수인프라 전력 안정적 확보",
          "과학기술기본법 개정 및 과학기술부총리·과학특임대사 신설"
        ]
      },
      "연구·인재 육성": {
        "title": "과학기술인이 존중받는 나라로 초격차 기술 혁신",
        "content": "조성, 육성, 강화 중심의 정책 추진",
        "details": [
          "과학기술인 처우 개선 및 위상 강화",
          "세계적 수준의 연구 중심 허브 육성 및 연구 몰입 환경 조성"
        ]
      }
    },
    "사회통합": {
      "청년 지원 및 기회 보장": {
        "title": "청년 취업, 주거, 교육, 결혼, 양육 부담 완화로 청년을 위한 기회 창출",
        "content": "도입, 지원 중심의 정책 추진",
        "details": [
          "청년안심 국민연금 2차 개혁으로 청년세대 부담 완화",
          "청년이 일하고 싶은 좋은 일자리 창출",
          "집 걱정 없는 청년 시대",
          "신혼부부 결혼 부담 완화",
          "육아부부 부담 덜기",
          "청년 도약 지원",
          "군가산점제 도입"
        ]
      }
    },
    "주거·교통": {
      "광역교통망 확장": {
        "title": "GTX를 수도권에서 전국 5대 광역권으로 확장하여 국토 균형발전 촉진",
        "content": "확대 중심의 정책 추진",
        "details": [
          "GTX 전국 5대 광역권 확대 추진"
        ]
      },
      "생활교통·혼잡 해소": {
        "title": "수도권 교통 체증을 해소하고 통근 시간을 획기적으로 단축하여 국민 삶의 질 개선",
        "content": "핵심 정책 추진",
        "details": [
          "광역철도, 도시철도 확충으로 30분 출퇴근 혁명 및 정주환경 대혁신"
        ]
      }
    },
    "복지·보건": {
      "출산·돌봄 복지": {
        "title": "인구구조 변화에 맞는 결혼·임신·출산·육아·교육 및 어르신 돌봄 강화",
        "content": "강화, 지원, 확대 중심의 정책 추진",
        "details": [
          "청년‧신혼부부 주거지원 확대, 내 집 마련 첫걸음 지원",
          "임신부터 육아까지 지원 확대",
          "우리 아이 자산 형성 지원",
          "디딤돌 소득 도입",
          "아이부터 어르신까지 예방접종 국가 지원 확대",
          "어르신 돌봄과 자립의 균형 체계 강화",
          "장애인 생애주기별 지원 강화"
        ]
      },
      "보건의료 개혁": {
        "title": "안심하고 누리는 더 좋은 보건의료",
        "content": "핵심 정책 추진",
        "details": [
          "의료안전망 복구와 합리적 의료시스템 재구축"
        ]
      }
    },
    "행정·정치개혁": {
      "재난·생활안전 체계 정비": {
        "title": "복합·대형화되는 재난에 대응력을 강화하고 사전대비 중심으로 전환",
        "content": "강화, 정비 중심의 정책 추진",
        "details": [
          "‘사후 수습’에서 ‘사전 대비’로 전환",
          "재난전문조직의 현장대응 역량 강화",
          "환경부를 기후환경부로 개편하여 기후재난에 선제 대응",
          "정부, 지자체, 민간 정보공유 플랫폼 강화",
          "국가 항공 건설·운영체제 전면 개편으로 항공 안전 혁신",
          "지하 안전사고 예방을 위한 싱크홀 대응 체계 구축",
          "하천지류 정비사업 신속 추진으로 자연재난 예방"
        ]
      },
      "정치·사법제도 개혁 및 신뢰 회복": {
        "title": "독립적 감사제도를 확립하여 공직사회 부정부패 일소",
        "content": "확대 중심의 정책 추진",
        "details": [
          "감사원 소속 감사관 파견·임명",
          "대공수사권 국정원 환원, 반국가세력 대응 역량 회복",
          "간첩법(형법 제98조) 개정, ‘적국’에서 ‘외국’으로 범위 확대",
          "국회의원 불체포 특권 폐지",
          "선관위 감사원 감사허용",
          "공수처 폐지",
          "사법 방해죄 신설"
        ]
      }
    },
    "외교·안보": {
      "국방·안보 강화": {
        "title": "북핵 억제력 강화로 국민이 안심하는 국방 구현",
        "content": "육성, 강화, 확대 중심의 정책 추진",
        "details": [
          "한미동맹 기반 핵 확장억제 실행력 강화",
          "현행 한국형 3축 체계를 더욱 강화하여 선제적 억제능력 확보",
          "핵 잠재력 강화",
          "‘글로벌 K-방산’ 육성하여 세계 시장 진출",
          "군 복무 여건 개선 및 수당 현실화, 헌신에 합당한 대우 실현",
          "군 복무 중 사상자에 대한 보상 및 보훈을 확대"
        ]
      },
      "외교·평화 전략": {
        "title": "미국과의 신뢰 관계를 바탕으로 ‘강한 대한민국’, ‘국민이 안심하는 대한민국’, ‘국제사회가 신뢰하는 대한민국’ 구현",
        "content": "핵심 정책 추진",
        "details": [
          "북한의 핵위협 가중시, ‘전술핵 재배치’ 또는 ‘NATO식 핵공유’를 한미간 협의",
          "핵추진 잠수함 개발 추진",
          "‘사이버전자전사령부’ 창설"
        ]
      }
    }
  },
  "lee-jaemyung": {
    "경제·산업": {
      "산업·성장 전략": {
        "title": "AI 등 신산업 집중육성을 통해 새로운 성장기반 구축",
        "content": "육성, 강화, 확대 중심의 정책 추진",
        "details": [
          "인공지능 대전환(AX)을 통해 AI 3강으로 도약",
          "K-방산을 국가대표산업으로 육성",
          "안정적 R&D 예산 확대 및 국가연구개발 지속성 담보",
          "스마트 데이터농업 확산, 푸드테크·그린바이오 산업 육성, K-푸드 수출 확대, R&D 강화, 농생명용지 조기 개발로 농업을 미래농산업으로 전환·육성",
          "재생에너지 중심 에너지전환",
          "에너지고속도로 구축",
          "탄소중립 산업전환",
          "건축물·열 부문 탈탄소화",
          "수송 부문 탈탄소화",
          "농업 탄소감축 추진",
          "탈플라스틱 로드맵 수립",
          "생물 다양성 복원",
          "4대강 재자연화 및 수질개선",
          "탄소 감축 실천 인센티브 강화",
          "정의로운 전환 실현 방안 마련",
          "UN 해양총회 유치"
        ]
      },
      "콘텐츠·문화산업 육성": {
        "title": "K-콘텐츠 지원강화로 글로벌 빅5 문화강국 실현",
        "content": "육성, 투자 중심의 정책 추진",
        "details": [
          "대한민국의 미래성장을 위한 글로벌 소프트파워 Big5 문화강국을 실현",
          "국가첨단전략산업에 대한 대규모 집중투자방안 마련",
          "벤처투자시장 육성으로 글로벌 4대 벤처강국 실현"
        ]
      },
      "소상공인·서민경제 지원": {
        "title": "가계와 소상공인의 활력을 제고",
        "content": "지원, 개선, 확대 중심의 정책 추진",
        "details": [
          "12.3 비상계엄으로 인한 피해 소상공인 지원방안 마련",
          "소비촉진으로 소상공인·자영업자 활기 도모",
          "소상공인·자영업자 사회안전망 확대",
          "플랫폼 중개수수료율 차별금지 및 수수료 상한제 도입",
          "취약계층 중금리대출 전문 인터넷은행 추진",
          "채무자 중심 보호체계 구축",
          "전세사기 걱정 없는 사회 구축",
          "디지털자산 생태계 정비",
          "중소기업 복지플랫폼 예산 확대",
          "기업지배구조 개선",
          "먹튀·시세조종 근절"
        ]
      },
      "공정경제·경제안보": {
        "title": "공정한 경제구조 실현",
        "content": "조성, 강화, 지원 중심의 정책 추진",
        "details": [
          "소상공인 금융과 경영부담 완화",
          "공정하고 지속가능하며 실패해도 재기할 수 있는 소상공인 경제 구축",
          "가맹점주·대리점주·수탁사업자·온라인플랫폼 입점사업자 등 협상력 강화",
          "대환대출 활성화 및 중도상환수수료 감면",
          "가산금리 산정 시 법적비용 부당전가 방지",
          "고품질 공공임대주택 및 공공임대 비율 확대",
          "주식시장 수급여건 개선 및 유동성 확충",
          "중소기업 단체협상권 부여",
          "기술탈취 강력 근절",
          "지배주주 사익편취 행위 근절",
          "국제적 통상환경 변화에 적극 대응하는 경제외교 추진",
          "우리의 외교영역을 확대하고 다변화",
          "국익과 실용의 기반 하에 주변 4국과의 외교관계 발전",
          "한반도 군사적 긴장 완화, 평화 분위기 조성",
          "국제사회에서의 공헌과 국격에 걸맞은 외교 추진",
          "재외국민에 대한 적극적인 지원",
          "수출산업 보호 위한 무역안보 단속체계 확립",
          "식량안보 차원의 농산물 안정적 공급기반 구축",
          "범죄로부터 안전한 사회 구축"
        ]
      }
    },
    "행정·정치개혁": {
      "정치·사법제도 개혁 및 신뢰 회복": {
        "title": "내란극복",
        "content": "조성, 강화, 개선 중심의 정책 추진",
        "details": [
          "대통령 계엄권한에 대한 민주적 통제 강화",
          "국민에 봉사하는 군으로 체질 개선",
          "검찰 개혁 완성",
          "주요 공공기관 기관장 등의 임기를 대통령 임기와 일치",
          "방송통신위원회의 정파성 극복을 위한 법제 정비",
          "제3기 진실화해위원회의 신속 출범",
          "정치보복 관행 근절 등 국민통합 추진",
          "반인권적으로 운영되고 있는 국가인권위원회의 정상화 추진",
          "사법 개혁 완수",
          "민생·인권친화적 제도 개선",
          "방송의 공공성 회복과 공적책무 이행",
          "학교 역사교육 강화 및 역사연구기관 운영의 정상화",
          "직접민주주의 강화 등을 통한 책임정치 구현",
          "감사원의 정치적 중립성 및 독립성 강화",
          "반부패 개혁으로 청렴한 공직문화 조성",
          "변호사의 공공성 강화",
          "건강하고 신뢰할 수 있는 미디어 이용환경 조성",
          "코로나 정책자금 대출에 대한 채무조정부터 탕감까지 종합방안 마련"
        ]
      },
      "재난·생활안전 체계 정비": {
        "title": "국민 생활안전 및 재난 대응 강화",
        "content": "조성, 보호, 개선 중심의 정책 추진",
        "details": [
          "민생침해 금융범죄 처벌 및 금융소비자 보호 제도 개선",
          "교통사고 예방 및 건설안전 체계 조성",
          "국민참여 의료개혁 추진 및 건강보험 지속가능성 확보"
        ]
      },
      "지방분권 및 지역 균형 강화": {
        "title": "세종 행정수도 완성",
        "content": "육성, 강화, 지원 중심의 정책 추진",
        "details": [
          "공공기관 지방이전 및 정주여건 개선",
          "자치분권 강화와 지방재정 확충",
          "지역대표 전략산업 육성 및 지역경제 활력 촉진",
          "지역사랑상품권 발행 의무화",
          "사람이 돌아오는 지속가능한 농산어촌",
          "지역중소방송사 콘텐츠 제작 지원",
          "모든 일하는 사람의 권리 보장",
          "균형발전 기반 마련",
          "지역소멸 방지를 위한 행정체계 개편",
          "수도권 대학 서열화 완화",
          "관광산업을 통한 지역경제 활성화",
          "철도지하화 계획 수립 및 시행",
          "이·통장 특별활동비 신설"
        ]
      }
    },
    "외교·안보": {
      "외교·평화 전략": {
        "title": "지속가능한 한반도 평화 실현",
        "content": "도입, 감축 중심의 정책 추진",
        "details": [
          "통상환경 변화 대응을 위한 무역구조 혁신",
          "북한 핵 위협의 단계적 감축 및 비핵·평화체제를 향한 실질적 진전 달성",
          "굳건한 한미동맹에 기반한 전방위적 억제능력 확보",
          "이산가족 상봉 등 남북 인도주의협력, 교류협력 모색·추진",
          "전략산업 국내생산 촉진세제 도입",
          "전략물자 국적선박 확보를 통한 물류 안보 실현",
          "불법 중국어선 강력 대응을 통한 해양 주권 수호"
        ]
      }
    },
    "복지·보건": {
      "보건의료 개혁": {
        "title": "의료 대란 해결 및 의료 개혁 추진",
        "content": "강화 중심의 정책 추진",
        "details": [
          "사회적 재난 대응체계 구축",
          "지역·필수·공공의료 강화",
          "세종 행정수도 완성 추진"
        ]
      },
      "연금·기초보장": {
        "title": "생활안정과 생활비절감 추진",
        "content": "강화, 지원, 확대 중심의 정책 추진",
        "details": [
          "돌봄기본사회 추진",
          "주거·통신 생활비 절감",
          "국가책임 공교육 강화",
          "생애주기 문화패스 확대",
          "어민 소득증대",
          "문화예술인 사회보장 강화",
          "1인가구·청년 대상 주거정책 확대",
          "교통물류 환경 개선",
          "동물복지 향상",
          "근로장려금·자녀장려금 확대",
          "교통비 절감 위한 패스 3종 도입",
          "청년 등록금·주거 지원",
          "농가소득 및 재해 안전망 도입",
          "국가유공자 예우 강화",
          "청년 기회·복지 확대",
          "한부모가족 지원 확대",
          "청년·근로자 아침밥 및 먹거리 돌봄 강화",
          "저출생 대책 및 자녀양육 지원 확대"
        ]
      },
      "출산·돌봄 복지": {
        "title": "저출생·고령화 해소 및 돌봄체계 구축",
        "content": "강화, 지원, 감축 중심의 정책 추진",
        "details": [
          "돌봄·교육·일가정 양립 지원 강화",
          "장애인 맞춤형 지역돌봄체계 구축",
          "외로움 대응 정책",
          "고령사회 대응 통합지원체계 마련",
          "지속가능한 노후소득 보장체계 구축",
          "온실가스 감축목표 수립"
        ]
      }
    },
    "사회통합": {
      "노동권 및 권리 존중": {
        "title": "노동 존중, 일하는 사람들의 권리 존중 사회 실현",
        "content": "조성, 강화, 개선 중심의 정책 추진",
        "details": [
          "포괄임금제 금지 명문화",
          "동일노동 동일임금 기준 마련",
          "단체교섭협약 활성화",
          "직장 내 민주주의 및 노사자율 강화",
          "자영업자까지 산재보험 도입",
          "노동안전보건체계 구축",
          "일하는 여성이 일하기 좋은 사회 조성",
          "주 4.5일 도입 및 실노동시간 단축",
          "공무원 처우개선 및 공직문화 개선",
          "문화예술인 창작권 보장",
          "장애인 권리보장 및 사회참여 실현",
          "생애주기별 소득보장체계 구축"
        ]
      }
    }
  },
  "lee-junseok": {
    "행정·정치개혁": {
      "정부조직 개편 및 행정 효율화": {
        "title": "부처 간 소관 분야 중복과 행정의 칸막이 문제를 최소화하여 효율적이고 전문화된 정부 운영",
        "content": "도입 중심의 정책 추진",
        "details": [
          "부처 개편 및 축소 (19부처 → 13부처)",
          "정부기구 효율화",
          "3부총리제 도입",
          "예산 편성 기능 이관"
        ]
      },
      "지방분권 및 지역 균형 강화": {
        "title": "지역 간 재정 자립도 제고 및 실질적인 지방 분권 강화",
        "content": "핵심 정책 추진",
        "details": [
          "지방소비세 이전 효과 극대화",
          "지역 실정 반영한 최저임금제도 운영",
          "적용 기준 명확화"
        ]
      }
    },
    "경제·산업": {
      "리쇼어링·지역 산업 활성화": {
        "title": "해외로 이전한 국내 기업의 국내 주요 산업단지 리쇼어링 촉진을 통해 지역 경제 활성화 및 러스트벨트 해소",
        "content": "강화, 지원 중심의 정책 추진",
        "details": [
          "리쇼어링 기업 지원을 위한 특별 규제완화",
          "외국인 노동자 전용 특수 비자 신설",
          "외국인 노동자 정착 지원 강화",
          "법인세 국세분의 일부를 지방세로 전환",
          "지자체별 법인세 경쟁 체계 구축",
          "최저임금 결정 권한의 지방 이양"
        ]
      },
      "기업환경 및 규제개혁": {
        "title": "글로벌 선도국가의 규제 수준을 벤치마킹하여 국내 규제를 획기적으로 혁신",
        "content": "도입, 확대 중심의 정책 추진",
        "details": [
          "규제기준국가제 도입",
          "규제심판원 설치 및 절차 통합",
          "특례 기간 확대 및 안정성 확보",
          "사업자단체 중심의 집단 신청 허용",
          "분야별 규제기준국가 설정 및 법령 개정 로드맵 마련"
        ]
      }
    },
    "복지·보건": {
      "연금·기초보장": {
        "title": "국민연금 구조개혁을 통한 지속 가능한 연금체계 구축 및 세대 간 형평성 확보",
        "content": "도입 중심의 정책 추진",
        "details": [
          "신-구 연금 재정 분리",
          "구연금 미적립부채 관리 방안 시행",
          "확정기여형 구조의 신연금 도입"
        ]
      }
    },
    "교육·과학기술": {
      "교권·교육": {
        "title": "교사에 대한 무분별한 소송과 허위신고로부터 교권과 학습권을 보호",
        "content": "도입, 강화, 지원 중심의 정책 추진",
        "details": [
          "교사 소송 국가책임제 도입 및 전담 변호사 배치",
          "교사에 대한 허위신고 무고 처벌 강화",
          "학습지도실 및 학생생활지원관 제도 도입",
          "단계적 분리조치 및 사후 적응 프로그램 운영"
        ]
      },
      "연구·인재 육성": {
        "title": "우수 과학기술인을 국가 차원에서 예우함으로써 연구자 자부심 회복 및 인재 유출 방지",
        "content": "도입 중심의 정책 추진",
        "details": [
          "(과학기술)연구자 연금 제도 도입",
          "과학기술 패스트트랙 제도 도입"
        ]
      }
    },
    "사회통합": {
      "청년 지원 및 기회 보장": {
        "title": "자산 형성과 사회 진출의 시기에 있는 청년에게 초기 자금 지원을 통해 실질적 기회의 평등 실현",
        "content": "핵심 정책 추진",
        "details": [
          "정책금융상품 ‘든든출발자금’ 출시",
          "상환 방식의 유연성 보장",
          "기존 학자금대출과의 선택 및 전환 가능",
          "용도 제한 및 사후 관리 체계"
        ]
      },
      "군복무 제도 및 병역 연계 개선": {
        "title": "병사 중심의 장교·부사관 선발 구조를 통해 인력 기반 확대 및 복무 유인 강화",
        "content": "도입, 지원 중심의 정책 추진",
        "details": [
          "병사·부사관·장교 기초군사훈련 통합",
          "단기복무 간부 선발제도 도입 및 교육과정 운영",
          "복무 연계 등록금 지원 제도 도입",
          "복무 유예제 도입"
        ]
      }
    }
  }
}

/**
 * 지역별 공약 데이터
 * 각 후보별 지역 공약 정보를 담고 있습니다.
 */
export const regionalPolicies = {
  capital: [
    {
      "candidateId": "lee-jaemyung",
      "name": "이재명",
      "policies": [
        {
          "icon": "ri-check-line",
          "text": "해당 지역 공약 정보가 없습니다."
        }
      ]
    }
  ],
  gangwon: [
    {
      "candidateId": "lee-jaemyung",
      "name": "이재명",
      "policies": [
        {
          "icon": "ri-check-line",
          "text": "강원도와 수도권은 하나의 생활권\n\n동서고속철도(춘천-속초)와 동해북부선(강릉-제진) 적기 완공\nGTX-B(마석/가평→춘천) 춘천 연장, GTX-D(김포·인천→팔당·원주) 원주 연장 적극 추진\n강원내륙고속도로(양구-영월-영천)와 동서평화고속도로(강화-고성) 적극 추진"
        },
        {
          "icon": "ri-check-line",
          "text": "강원특별자치도 권한과 자율성 대폭 확대\n\n'강원특별법' 3차 개정안 국회 통과 지원\n강원도 직접 기획 강원형 자치모델 발전\n교육, 산업, 국토계획, 환경 등 실질적 권한 이양\n강원도 주도 산업 및 인재 육성"
        },
        {
          "icon": "ri-check-line",
          "text": "미래산업 선도도시, 강원\n\n동해안, 환동해 경제권 전략 거점 육성 북방교역 활성화\n부유식 해상풍력과 수소에너지 산업 육성\nAI·디지털 기반 첨단의료복합 산업 육성 (춘천·원주·강릉)\n반도체 교육·소재·부품 산업 육성 (춘천·원주·강릉)\n접경지 평화경제특구 조성 (철원·화천·양구·인제·고성 등)\n폐광지역 성장 기반 구축(태백·삼척·영월·정선 등) 청정에너지, 의료·관광 등 미래산업 육성"
        },
        {
          "icon": "ri-check-line",
          "text": "세계로 뻗는 K-문화관광벨트, 강원\n\n올림픽 유산 활용 K-문화·스포츠 복합 관광지 조성 평창·강릉 등 K-팝·스포츠·자연 융합형 체험 관광지 조성\n동해안과 강원 등 세계적 관광특구 육성 복합 해양레저, 산림휴양, 의료·힐링 관광 개발\n접경지·DMZ·생태·관광·평화 콘텐츠 개발"
        },
        {
          "icon": "ri-check-line",
          "text": "인구로 커가는 강원, 안전으로 지켜가는 강원\n\n청년형 블루시티 조성 및 은퇴자 정주 단지 조성 춘천·원주·평창 등 미니 신도시형 은퇴자 정주 단지 조성\n주거·의료·문화·일자리를 갖춘 복합 커뮤니티 조성\n최고 수준 강원형 통합 재난 대응 시스템 구축 닥터헬기 추가 도입(영서→춘천, 영동→강릉)산간 지역 응급이송 체계 구축\n소방헬기 확충 산불·재난 대응 능력 대폭 강화"
        }
      ]
    }
  ],
  jeju: [
    {
      "candidateId": "lee-jaemyung",
      "name": "이재명",
      "policies": [
        {
          "icon": "ri-check-line",
          "text": "탄소중립, 녹색 문명의 미래 제주\n기후 위기, 지방 소멸, 산업전환의 해법을 제주가 선도\n탄소중립, 재생에너지, 관광, 의료 산업이 함께 숨 쉬는 제주\n평화·자연·생명의 섬, 제주"
        },
        {
          "icon": "ri-check-line",
          "text": "자원순환 100%, 탄소중립 섬 제주\n해상풍력·태양광 기반 청정 전력망 구축\n전기차 100%, 탄소제로 주택 보급\n‘햇빛·바람 연금’으로 에너지 수익 환원"
        },
        {
          "icon": "ri-check-line",
          "text": "일과 쉼, 스포츠와 관광이 공존하는 사계절 체류형 제주\n국제 전지훈련센터, 재활 클리닉 조성\n요트·서핑 등 해양레저 인프라 확대\n디지털 인프라 구축과 읍면동 특화 관광 콘텐츠 연계\n‘제주 4·3 아카이브 기록관’ 건립"
        },
        {
          "icon": "ri-check-line",
          "text": "의료·생명 산업 제주\n제주대병원 상급종합병원 격상 추진\n천연 바이오 자원 DB화, 신약 개발 기반 조성\n제주형 바이오 헬스 클러스터 완성"
        },
        {
          "icon": "ri-check-line",
          "text": "디지털 전환, 미래 농업, 높은 농가 소득 제주\n디지털 농업 플랫폼 구축\n농산물 스마트 가공센터 적극 지원\n해상운송비 부담 완화 정책 병행 추진"
        }
      ]
    }
  ],
  yeongnam: [
    {
      "candidateId": "lee-jaemyung",
      "name": "이재명",
      "policies": [
        {
          "icon": "ri-check-line",
          "text": "해양수산부를 부산으로 이전하고 부산을 해양강국의 중심으로!\n해양수산부와 해양 관련 공공기관 부산으로 이전\n해운물류 대기업 본사 부산 이전, 해사법원 신설 추진\n부산을 e-스포츠 중심지로 육성"
        },
        {
          "icon": "ri-check-line",
          "text": "부울경을 육해공 ‘트라이포트’ 전진기지로 구축!\n가덕도 신공항과 동남권 철도 사업 차질 없이 준비\n북극항로 개척으로, 부울경을 육해공 물류 융합 중심지로 육성\n북극항로 전용 선박건조 지원, 국제 해기사 등 인력 양성\n극지항로 최적화 인프라 구축\n트라이포트 배후단지에 글로벌 소부장 산업 집적지 육성"
        },
        {
          "icon": "ri-check-line",
          "text": "울산을 글로벌 자동차·석유화학·조선 산업의 중심지로!\n완성차 기업·부품업체의 미래차 전환과 고용 전환 지원\n석유화학산업의 고부가가치 산업구조 전환 지원\n조선산업 자율운항, 친환경 선박기술 개발 지원, 부유식 해상풍력 단지 조성 추진"
        },
        {
          "icon": "ri-check-line",
          "text": "경남을 첨단 스마트 산업, 우주·항공·방산, 스마트 조선 산업의 메카로 조성!\n경남산업단지에 AI 기반 첨단스마트 공장 확산, 중소기업 AI활용 클라우드 기반 인프라 구축\n경남을 글로벌 G4 방산 집적지로 육성\n고부가가치 선박 경쟁력 강화, 중소형 조선사 선박금융 지원 확대"
        },
        {
          "icon": "ri-check-line",
          "text": "‘부울경 30분대 생활권’ 조성으로 1천만 메가시티를 앞당기겠습니다!\n부전~마산 복선전철 조기 개통\n부산~양산~울산 광역철도 건설 지원\n부전역을 동남권의 중심역으로 격상\n거가대교, 마창대교 등 민자사업 재구조화로 이용자 부담 완화"
        }
      ]
    },
    {
      "candidateId": "lee-jaemyung",
      "name": "이재명",
      "policies": [
        {
          "icon": "ri-check-line",
          "text": "대형산불 실질적인 피해복구와 보상\n대형산불재난 대응체계 전면 재구축"
        },
        {
          "icon": "ri-check-line",
          "text": "경주APEC 정상회의 성공적인 개최\n회의장, 숙소 리모델링, 미디어센터 등 인프라 확충지원\n경주 APEC이 지역경제 회복의 계기가 되도록 준비\nK-민주주의 회복력 알리고, 민주주의 선도국 위상 회복"
        },
        {
          "icon": "ri-check-line",
          "text": "이차전지 산업벨트 - 미래 자동차부품 클러스터 조성\n차세대 전고체 배터리, 재활용·리사이클링 기술개발 지원\n이차전지 규제 합리화, 매력적인 투자 환경 조성\n대구·경북 자동차부품업체 친환경차 첨단부품 업종으로 전환 지원\n자동차부품업체 친환경차 전환 금융 및 세제, 교육지원"
        },
        {
          "icon": "ri-check-line",
          "text": "미래 신성장 동력으로 바이오산업 집중 육성\n한국형 바이오·백신산업 클러스터 육성\n바이오 전문인력 양성 지속 확대\n농업, 의료, 바이오헬스, 식품 등 바이오 전환 산업 육성\n의료 소외지역 경북 북부권 의료서비스 강화, 바이오 산업과 연계"
        },
        {
          "icon": "ri-check-line",
          "text": "AI로봇, 수소산업, 고부가 가치 섬유산업 육성\n대구 AI로봇기업 글로벌 진출 지원, ‘딥테크(첨단·혁신·기술)’ 유니콘 육성\n포항 수소·첨단·신소재 특화지구 조성·발전\n수소산업 전환 지원, 수(水)전해 수소생산 설비 산업 육성\n대구 섬유산업 고부가가치 산업으로 고도화 지원"
        },
        {
          "icon": "ri-check-line",
          "text": "대구경북 통합신공항, 울릉공항 성공적 추진\n통합 신공항 사업추진 지원 요소 조속히 해결\n울릉공항 조속한 완공과 안전성 확보 지원\n포항이 북극항로 기항지 되도록 항만 확충 사업 지원"
        },
        {
          "icon": "ri-check-line",
          "text": "대구경북 내 동서남북 교통망 구축\n중부권 동서횡단철도 신속 추진(문경~천안~서산)\n신공항 순환철도(대구~신흥~의성~구미~영천~대구),\n  대구경북 대순환철도(대구~구미~고령~문경~영주~봉화~울진~포항~영천~대구) 추진\n남북잇는 대선봉덕선(청송~양양~홍천)과 영월~정선~평창~횡성~인제~양구), 남북잇는 고속도로 국토계획 반영\nKTX이음/고속철 정차역 대구 도심구간 경부선 지하화 단계적 추진"
        }
      ]
    }
  ],
  chungcheong: [
    {
      "candidateId": "lee-jaemyung",
      "name": "이재명",
      "policies": [
        {
          "icon": "ri-check-line",
          "text": "세종시 행정수도 완성\n국회 세종의사당, 대통령 집무실 건립 조기추진\n행정기능과 대통령 집무실 이전 사회적 합의로 추진\n중단된 공공기관 이전 조속 재개\n충남·대전혁신도시 지역 정병력 고려 공공기관 유치\n충청권메가시티 중부내륙지주의 성장 거점으로 육성"
        },
        {
          "icon": "ri-check-line",
          "text": "대전을 과학수도로 도약!\n글로벌 융합연구 네트워크 강화\n인재 양성 인프라 지원\n산·학·관 R&D에서 대폭 확대\n연구인력·기술이 정주여건 개선 및 시작되지 않은 성과 지원 확대\n무늬년 연구환경 생태계 복원"
        },
        {
          "icon": "ri-check-line",
          "text": "충청권 첨단 산업벨트 구축 (03-1)\n대전광역시: 대덕연구특구 AI우주산업 중심지\n세종특별시: 스마트시티 국가시범도시 전략 연계, 스마트/디지털 행정 허브로 조성"
        },
        {
          "icon": "ri-check-line",
          "text": "충청권 첨단 산업벨트 구축 (03-2)\n충청북도: K-바이오 스퀘어 조기 조성, 글로벌 바이오 생산 클러스터 육성, 이차전지·반도체 산업 조성\n충청남도: 국회 경관청, 생산성 갖춘 디스플레이 산업 메카로 육성, 자동차, 반도체, 태양광 산업 클러스터 육성, 친환경 산업단지 재편\n    (노후 산단, 탄소 섬유 식단화학단지 폐쇄), 내포신도시 중심 특수통신서비스지구\n보령시, 계룡에 국방 관련 기관 유치하여 스마트 국방 산업 발전 지원"
        },
        {
          "icon": "ri-check-line",
          "text": "환황해권 해양관광벨트 및 충북 유양·힐링 관광벨트 조성\n서해안지역 생태복원으로 환황해권 해양관광벨트 조성\n가로림만 해양정원 조성 및 해양교량 건설\n서천 블루카본, 저지대복원 생태관광\n서산 간월호, 천수만 해수유통 생태 복원\n금강·삽교천 하류 유역의 자연성 회복 지원\n충북 내륙·산림자원 활용한 관광벨트 조성\n연풍온천~속리산~괴산~보은 생태휴양벨트\n단양~제천~수안보~문경~예천~태백 관광벨트 조성 지원"
        },
        {
          "icon": "ri-check-line",
          "text": "청주공항 확장! 사통팔달 광역교통망 구축\n청주공항 민간 전용활주로 신설하여 여객 수용력과 운항율↑\n충북권 통과 철도망(동남 서남~충북 중심~경북 교류) 조기 확장\nX자형 강호축 철도망 구축\n청주광역 급행철도(CTX) 적기 착공\nGTX 천안, 아산 연장 조속 추진\n수도권 내륙선 등 충청권 광역철도 조기 구축\n제2세종대교 건설 적극 검토"
        }
      ]
    }
  ],
  border: [
    {
      "candidateId": "lee-jaemyung",
      "name": "이재명",
      "policies": [
        {
          "icon": "ri-check-line",
          "text": "군사적 긴장 완화와 남북 신뢰 복원 시급\n9.19 군사합의 복원\n대북전단, 오물풍선, 대북·대남 방송 상호 중단 추진\n남북군사공동위원회 구성 및 소통 채널 복원\n남북관계 리스크 관리, 남북 교류·협력 재개 모색"
        },
        {
          "icon": "ri-check-line",
          "text": "접경지역 주민 보상 위한 법령 시행 및 예산 집행 신속 추진\n'민방위기본법' 개정안 시행 통한 주민 보상 신속 추진\n방음시설 설치 지원"
        },
        {
          "icon": "ri-check-line",
          "text": "특구 지정으로 접경지역 경제 살리기\n‘평화경제특구’ 및 ‘기회발전특구’ 지정 적극 검토\n인프라 확충, 산업 역량 강화, 신산업 육성"
        },
        {
          "icon": "ri-check-line",
          "text": "규제 개선 통한 접경지역 주민 재산권 보장\n군사시설보호구역 합리적 조정\n작전상 불필요한 군 방호벽 철거 추진\n규제 개선 관련 행정절차 간소화"
        }
      ]
    }
  ],
  honam: [
    {
      "candidateId": "lee-jaemyung",
      "name": "이재명",
      "policies": [
        {
          "icon": "ri-check-line",
          "text": "AI, 미래 모빌리티, 금융도시로 혁신하는 호남\n광주를 AI·미래 모빌리티 산업 선도도시로 육성\n전주를 제3 금융 중심지로 추진\n여수·광양산업단지를 친환경·고부가산업으로 전환\n새만금 첨단산업 특화단지에 이차전지 기업 유치 지원\n목포는 해상풍력 및 고지식산업 중심지로\n군산은 조선소 재도약"
        },
        {
          "icon": "ri-check-line",
          "text": "재생에너지 산업의 메카, 에너지가 소득이 되는 호남\n해상풍력·태양광 재생에너지 산업 기반 조성\n호남 전역에 RE100 산업단지 구축\n주민 참여형 ‘햇빛·바람연금’확산\n에너지 고속도로로 주요 산업단지 연결"
        },
        {
          "icon": "ri-check-line",
          "text": "농·축·수산의 미래, K-푸드가 성장하는 호남\nAI 농업, 친환경 농기계 도입\n스마트팜, 자원 순환형 축산시스템\n수산·식품 클러스터 조성"
        },
        {
          "icon": "ri-check-line",
          "text": "전남·북 의대 신설! 바이오 인재와 산업이 자라는 호남\n전남·전북에 국립의대 설립으로 인력양성·의료 사각지대 해소\n화순 백신 특구를 백신·항암·면역 중심지로 도약"
        },
        {
          "icon": "ri-check-line",
          "text": "전통과 콘텐츠가 만나는 글로벌 문화 관광 도시 호남\n2036년 전주 올림픽 유치, 전폭 지원!\n전주와 광주를 아시아 콘텐츠 허브 도시로!\n서남해안 해양·치유 관광, 내륙 생태·전통·농촌 관광 활성화"
        },
        {
          "icon": "ri-check-line",
          "text": "촘촘한 철도·고속도로 연결로 빠르고 강한 호남\n광주와 전남·전북을 하나로 잇는 촘촘한 교통망 구축\n수도권과 영남을 빠르게 연결하는 광역 교통망 구축\n호남선고속철 2단계 조기 완공, 전라선 고속철도 신속 추진\n서해선 고속철 추진(고속철-목포 연결 추진), 환승 체계 지원\n전남내 미개설 고속도로(고흥~광주·전주~세종) 국가계획 반영\n광주~대구 달빛철도 및 전북~대구 내륙철도 조속 추진\n새만금~포항 고속도로 전 구간 개통 지원\n호남선 전철화·고속화, 교통약자 이동권 보장 확대\n부안~고창 등 낙후지역 신규 고속도로망 입주 교통망 구축\n광주 군 공항 이전 지원"
        }
      ]
    }
  ]
};

// /**
//  * 후보자 발언 데이터
//  * 각 후보별 주요 발언과 인터뷰 내용을 담고 있습니다.
//  */
// export const statements: Statement[] = [
//   // 이재명 후보 발언
//   {
//     id: 'lee-jaemyung-statement-1',
//     candidateId: 'lee-jaemyung',
//     content: '경제 회복과 일자리 창출이 최우선 과제입니다. 중소기업과 소상공인의 경쟁력 강화를 통해 지역 경제를 활성화하고, 양질의 일자리를 만들어 나가겠습니다.',
//     source: 'KBS 토론회',
//     date: '2025년 4월 15일'
//   },
//   {
//     id: 'lee-jaemyung-statement-2',
//     candidateId: 'lee-jaemyung',
//     content: '청년들이 꿈을 펼칠 수 있는 환경을 만드는 것이 중요합니다. 청년 창업 지원과 주거 안정 정책을 통해 청년들의 미래를 지원하고, 저출산 문제 해결에도 기여하겠습니다.',
//     source: 'MBC 인터뷰',
//     date: '2025년 4월 20일'
//   },
//   {
//     id: 'lee-jaemyung-statement-3',
//     candidateId: 'lee-jaemyung',
//     content: '교육은 미래를 위한 투자입니다. 지역 간 교육 격차를 해소하고, 모든 학생들이 질 높은 교육을 받을 수 있도록 공교육을 강화하겠습니다.',
//     source: '교육정책 토론회',
//     date: '2025년 4월 22일'
//   },
  
//   // 다른 후보들의 발언은 추가해야 함
//   // 나머지 후보들의 발언을 각자 ID에 맞게 추가해주세요
// ];

// /**
//  * 후보자 Q&A 데이터
//  * 각 후보별 인터뷰 및 토론회 질의응답 내용을 담고 있습니다.
//  */
// export const qnas: QnA[] = [
//   // 이재명 후보 Q&A
//   {
//     id: 'lee-jaemyung-qna-1',
//     candidateId: 'lee-jaemyung',
//     question: '당선되면 가장 먼저 추진할 정책은 무엇인가요?',
//     answer: '경제 회복과 일자리 창출을 위한 정책을 가장 먼저 추진하겠습니다. 특히 코로나19로 어려움을 겪고 있는 중소기업과 소상공인을 위한 지원 정책을 강화하고, 청년 일자리 창출을 위한 정책을 적극적으로 추진하겠습니다. 또한 디지털 전환 지원을 통해 기업의 경쟁력을 강화하고, 새로운 산업 생태계를 조성하여 지속가능한 경제 성장의 기반을 마련하겠습니다.'
//   },
//   {
//     id: 'lee-jaemyung-qna-2',
//     candidateId: 'lee-jaemyung',
//     question: '지역 경제 활성화를 위한 구체적인 방안은 무엇인가요?',
//     answer: '지역 경제 활성화를 위해 다양한 정책을 추진하겠습니다. 첫째, 지역 화폐 활성화를 통해 지역 내 소비를 촉진하고 지역 경제 순환을 강화하겠습니다. 둘째, 지역 특화 산업 육성을 통해 지역 경제의 경쟁력을 강화하겠습니다. 셋째, 소상공인 디지털 전환 지원을 통해 온라인 판로를 확대하고 경쟁력을 강화하겠습니다. 넷째, 지역 관광 자원 개발을 통해 관광 산업을 활성화하고 지역 경제에 활력을 불어넣겠습니다.'
//   },
  
//   // 다른 후보들의 Q&A는 추가해야 함
//   // 나머지 후보들의 Q&A를 각자 ID에 맞게 추가해주세요
// ];