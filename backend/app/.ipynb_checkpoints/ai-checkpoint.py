...
def search_documents(query: str, candidate=None, k=5):
    ...
    """
    지윤
    어디에 넣어야할지 몰라서 여기에 넣음
    """
    
    except_dict={
        "이재명": ["김문수", "이준석", "다른 사람", "비교", "차이", "다른 후보"],
        "김문수": ["이재명", "이준석", "다른 사람", "비교", "차이", "다른 후보"],
        "이준석": ["김문수", "이재명", "다른 사람", "비교", "차이", "다른 후보"]
    }
    except_pattern=except_dict[candidate]
    pattern = re.compile("|".join(map(re.escape, except_pattern)))
    
    try:
        # 비교 또는 다른 후보자에 대한 언급이 질문에 있을 때는 필터 안하고 검색
        if pattern.search(s):
            ...
            if candidate:
                ...
                retriever.search_kwargs["filter"] = meta_filter
            ...
            for doc in results:
                ...
                if hasattr(doc, 'metadata') and doc.metadata:
                    ...
                    if not source_metadata:
                        ...
                        source_name = metadata.get('source_name', DEFAULT_SOURCE)
